...
///**********************************************************
//* 函数功能 ---> 获取芯片ID
//* 入口参数 ---> u32ChipUniquelID 返回ID的值
//* 返回数值 ---> none
//* 功能说明 ---> 96位ID是STM32唯一的身份标识，可以以8bit,
//*               16bit,32bit读取，提供了大端和小端两种表示方法。
//*               STM32F10X的地址是0x1FFFF7E8
//*               STM32F20X的地址是0x1FFF7A10
//*               STM32F30X的地址是0x1FFFF7AC
//*               STM32F40X的地址是0x1FFF7A10
//*               STM32L1XX的地址是0x1FF80050
//**********************************************************/
void Get_ChipID(uint32_t *u32ChipUniquelID)
{
    //u32 l_u32ChipUniquelID[3];
    #ifdef STM32F2XX_HD
    uint32_t *l_pu32ChipIDAddr = (uint32_t *)((__IO uint32_t*)(0x1FFF7A10));
    #elif (defined(N32G45X))
    uint32_t *l_pu32ChipIDAddr = (uint32_t *)((__IO uint32_t*)(0x1FFFF7F0));
    #elif (defined(GD32F4XX))
    uint32_t *l_pu32ChipIDAddr = (uint32_t *)((__IO uint32_t*)(0x1FFF7A10));
    #else
    uint32_t *l_pu32ChipIDAddr = (uint32_t *)((__IO uint32_t*)(0x1FFFF7E8));
    #endif
    //u32ChipUniquelID[2] = *(__IO uint32_t*)(0x1FFFF7E8);   // 低字节
    //u32ChipUniquelID[1] = *(__IO uint32_t*)(0x1FFFF7EC);   // 中字节
    //u32ChipUniquelID[0] = *(__IO uint32_t*)(0x1FFFF7F0);   // 高字节

    u32ChipUniquelID[2] = *l_pu32ChipIDAddr;                // 低字节
    u32ChipUniquelID[1] = *(l_pu32ChipIDAddr+1);            // 中字节
    u32ChipUniquelID[0] = *(l_pu32ChipIDAddr+2);            // 高字节
}
