...
#define SIZE_UART_RECEIVE_CMD                       1500
...
void Uart2ParserMethod(void)
{
    ...
    if(pUartCommVar->bRecCmdFlg)
    {
        //dbg_printf("Cmd:%s",pUartCommVar->pRecBuffer);
        {
            u16 i;
            dbg_printf("Cmd:");
            for(i=0;i<pUartCommVar->u16RecCnt;i++)
            {
                dbg_printf("0x%02x ",pUartCommVar->pRecBuffer[i]);
            }
            dbg_printf("\r\n");
        }
...
#if 0//_ENABLE_MULTI_COMMAND_MATCH_FUNCTION
