...
{
    mapp_MUH0808S_PlatformTimerSet(emUserTimer_LcdBacklightTimeout,15000);
...
void mapp_LCDBacklightMuteProcess(void)
{
    ...
    {
        if(mapp_MUH0808S_PlatformTimerExpired(emUserTimer_LcdBacklightTimeout))
        {
...
void AllOutKeyStaHandle(void)
{
    ...
    for(i=0;i<Def_outport_num;i++)
    {
        ...
        if(IN_channel>0)
        {
            app_MUH0808SSourceSwitch(i,IN_channel-1);
...
void EnterSwitchStatusHandle(void)
{
    ...
    else
    {
        for ( i = 0; i < KeyBuffValue.OutputCnt; i++ )
        {
            ...
            if (OUT_channel[i] > 0)
            {
                app_MUH0808SSourceSwitch(OUT_channel[i]-1,IN_channel-1);
...
void SceneStatusHandle(void)
{
    ...
    if(pstBasicVar->stScenePollingVar.bPollingEnable)
    {
        ...
        USER_printf("scene polling disable ok\r\n");
...
void EnterSceneCallStatusHandle(void)
{
    ...
    if(IN_channel<=32&&IN_channel>=1)
    {
        ...
        {
            app_MUH0808SSceneCall(IN_channel-1);

            USER_printf("scene %d called ok\r\n",IN_channel);
        }
    }
...
void EnterSceneSaveStatusHandle(void)
{
    ...
    if(IN_channel<=32&&IN_channel>=1)
    {
        app_MUH0808SSceneSave(IN_channel-1);
        USER_printf("scene %d saved ok\r\n",IN_channel);
    }
...
void LCD_RecoverToDefault(uint16_t u16Timeout)
{
    ...
    mapp_MUH0808S_PlatformTimerSet(emUserTimer_LcdRefreshTime,u16Timeout);
}

void LED_RecoverToDefault(uint16_t u16Timeout)
{
    UITimeOutBuff.LEDStar = 1;
    mapp_MUH0808S_PlatformTimerSet(emUserTimer_LedRefreshTime,u16Timeout);
}
...
void led_handle(void)
{
    ...
    {
        if(mapp_MUH0808S_PlatformTimerExpired(emUserTimer_LedRefreshTime))
        {
            for(i=led_name_LedInput1;i<led_name_End;i++)
            {
                mapp_SetLedState(i,FALSE);
    ...
    {
        if(mapp_MUH0808S_PlatformTimerExpired(emUserTimer_LcdRefreshTime))
        {
            pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
            pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
