...
static emCmdFuncStaType mapp_CmdfuncSetTxResolution(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8ResID>=1&&l_s8ResID<=emRes_max))
    {
        ...
        {
            l_u8RealPort = cg_u8ChannelTable[l_s8Output-1][1]+1;
            app_MS1826TxResolutionChange(0,l_u8RealPort-1,l_s8ResID-1);
...
static emCmdFuncStaType mapp_CmdfuncGetRxResolution(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(l_s8Input<=4)
    {
        if(l_s8Input == 0)
        {
            ...
            for(i=0;i<4;i++)
            {
                app_ms1826_get_rx_timing(0,cg_u8ChannelTable[i][1],&l_stRxTming);
        ...
        {
            app_ms1826_get_rx_timing(0,cg_u8ChannelTable[l_s8Input-1][1],&l_stRxTming);
...
static emCmdFuncStaType mapp_CmdfuncSwitchFreeze(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(*(l_pCmd+1)=='.')
    {
        ...
        {
            if(l_u8FreeSta&0x01)
                app_MS1826SwitchFreeze(0,cg_u8ChannelTable[0][1],45);
            if(l_u8FreeSta&0x02)
                app_MS1826SwitchFreeze(0,cg_u8ChannelTable[1][1],45);
            if(l_u8FreeSta&0x04)
                app_MS1826SwitchFreeze(0,cg_u8ChannelTable[2][1],45);
            if(l_u8FreeSta&0x08)
                app_MS1826SwitchFreeze(0,cg_u8ChannelTable[3][1],45);
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Brightness<=100))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                app_LMX_4IUHS_CPictureAdjust(i,(u8)l_s16Brightness,MS1826_ADJUST_BRIGHTNESS);
            }
        }
        else
        {
            ...
            app_LMX_4IUHS_CPictureAdjust(l_s8Output-1,(u8)l_s16Brightness,MS1826_ADJUST_BRIGHTNESS);
        }
        SetUserData_SpecBasicVar();
    }
    else
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=100))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                app_LMX_4IUHS_CPictureAdjust(i,(u8)l_s16Value,MS1826_ADJUST_CONTRAST);
            }
        }
        else
        {
            ...
            app_LMX_4IUHS_CPictureAdjust(l_s8Output-1,(u8)l_s16Value,MS1826_ADJUST_CONTRAST);
        }
        SetUserData_SpecBasicVar();
    }
    else
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=100))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                app_LMX_4IUHS_CPictureAdjust(i,(u8)l_s16Value,MS1826_ADJUST_SATURATION);
            }
        }
        else
        {
            ...
            pCommVar->SendPrintf("output %d saturation %d\r\n",l_s8Output,l_s16Value);
            app_LMX_4IUHS_CPictureAdjust(l_s8Output-1,(u8)l_s16Value,MS1826_ADJUST_SATURATION);
        }
        SetUserData_SpecBasicVar();
    }
    else
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=100))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                app_LMX_4IUHS_CPictureAdjust(l_u8RealPort-1,(u8)l_s16Value,MS1826_ADJUST_HUE);
            }
        }
        else
        {
...
static emCmdFuncStaType mapp_CmdfuncCheckOutputCardType(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(l_s8Num>0)
    {
        pCommVar->SendPrintf("MI%02d%s",l_s8Num,_STRING_CARD_TYPE);
        l_state = emCmdNone;
    }
    else
        l_state = emCmdError;
...
static emCmdFuncStaType mapp_CmdfuncCheckCardMode(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(l_s8Num>0)
    {
        pCommVar->SendPrintf("TI%02d%s",l_s8Num,_STRING_CARD_TYPE_NUM);
        l_state = emCmdNone;
    }
    else
        l_state = emCmdError;
...
static emCmdFuncStaType mapp_CmdfuncCheckSetEdidData(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    {
        pCommVar->SendPrintf("OKEND");
        app_MS1826RxEdidUpdate(0,0,(pu8)_6_1920x1080p60,256);
        app_MS1826RxEdidUpdate(0,1,(pu8)_6_1920x1080p60,256);
        app_MS1826RxEdidUpdate(0,2,(pu8)_6_1920x1080p60,256);
        app_MS1826RxEdidUpdate(0,3,(pu8)_6_1920x1080p60,256);
        memcpy(pstBasicVar->u8RxEdid[0],_6_1920x1080p60,256);
        memcpy(pstBasicVar->u8RxEdid[1],_6_1920x1080p60,256);
        memcpy(pstBasicVar->u8RxEdid[2],_6_1920x1080p60,256);
        memcpy(pstBasicVar->u8RxEdid[3],_6_1920x1080p60,256);

        SetUserData_SpecBasicVar();
    }
    else if((l_pCmd[0]>='0')&&(l_pCmd[0]<='9'))
    {
        l_s8Num = (l_pCmd[0]-0x30)*10+(l_pCmd[1]-0x30);

        if(l_s8Num>0)
        {
            l_s8Num = (l_s8Num-1)%4+1;
            if(is_edid_valid((pu8)(l_pCmd+2)))
            {
                l_u8RealPort = cg_u8ChannelTable[l_s8Num-1][0]+1;
                memcpy(pstBasicVar->u8RxEdid[l_s8Num-1],(pu8)(l_pCmd+2),256);
                app_MS1826RxEdidUpdate(0,l_u8RealPort-1,(pu8)(l_pCmd+2),256);
                pCommVar->SendPrintf("OKEND");
...
bool app_SwitchResolution(u8 u8Tx,u8 u8ResID)
{
    ...
    switch(u8ResID)
    {
        ...
        case 4:     //1280x720p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1280x720_60);
            }
            break;
        case 5:     // 1280x720p59
            {
        ...
        case 6:     //1280x720p50
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1280x720_50);
            }
            break;
        case 7:     //1280x720p30
            {
        ...
        case 13:    //1920x1080p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1920x1080_60);
            }
            break;
        ...
        case 15:    //1920x1080p50
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1920x1080_50);
            }
            break;
        ...
        case 24:    //1024x768p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1024x768_60);
            }
            break;
        case 25:    //1024x768p75
            {
        ...
        case 26:    //1280x1024p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1280x1024_60);
            }
            break;
        ...
        case 28:    //1360x768p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1360x768_60);
            }
            break;
        case 29:    //1366x768p60
            {
        ...
        case 30:    //1400x1050p60
            {
                l_bRetVal = TRUE;
                //app_MS1826TxResolutionChange(u8Tx%4-1,emRes_1400x900_60);
            }
            break;
        case 31:    //1600x1200p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1600x1200_60);
            }
            break;
        case 32:    //1440x900p60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1440x900_60);
            }
            break;
        case 33:    //1440x900p75
            {
        ...
        case 37:    //1920x1200P60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1920x1200_60);
            }
            break;
        case 38:
            {
        ...
        case 39:    //1600x900P60
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_1600x900_60);
            }
            break;
        case 40:    //3840x2160 30Hz
            {
                ...
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_3840x2160_30);
            }
            break;
        case 44:    //4096x2160 30Hz
            {
                #if _ENABLE_RES_4096x2160_30
                l_bRetVal = TRUE;
                app_MS1826TxResolutionChange(0,(u8Tx-1)%4,emRes_4096x2160_30);
                #endif
            }
            break;
        default:
...
void app_Type1CmdFunction(u8 u8Port,u8 u8Cmd)
{
    ...
    switch(u8Cmd)
    {
        case 3:
            ...
            app_MS1826TxConfig(0,u8Port-1,pstBasicVar->u8TxFormatSta[u8Port-1]);
            SetUserData_SpecBasicVar();
            break;
        case 4:
        ...
        case 6:
            ...
            app_MS1826TxHdcpEnable(0,u8Port-1,pstBasicVar->u8TxHdcpMode[u8Port-1]);
            SetUserData_SpecBasicVar();
            break;
        case 7:
        ...
        case 10:
            ...
            app_MS1826TxDigitalAudioMute(0,u8Port-1,pstBasicVar->u8AudioMute[u8Port-1]);
            SetUserData_SpecBasicVar();
            break;
        case 11:
        ...
        case 16:
            ...
            app_MS1826TxPatternConfig(0,u8Port-1,1,0);
            mapp_SetAudioMute(u8Port-1,FALSE);
            //SetUserData_SpecBasicVar();
            break;
        case 17:
            ...
            dbg_printf("out %d black screen %s\r\n",u8Port,"disable");
            app_MS1826TxPatternConfig(0,u8Port-1,1,1);
            mapp_SetAudioMute(u8Port-1,TRUE);
            //SetUserData_SpecBasicVar();
            break;
        case 18:
            ...
            app_MS1826FreezeChange(0,u8Port-1,0);
            break;
        case 19:
            dbg_printf("output %d freeze %s\r\n",u8Port,"on");
            pstBasicVar->u8FreezeSta[u8Port-1] = 1;
            app_MS1826FreezeChange(0,u8Port-1,1);
            break;
...
void app_Type2CmdFunction(u8 u8Port,u8 u8Cmd)
{
    ...
    if(u8Cmd<=100)
        pstBasicVar->u8Brightness[u8Port-1] = u8Cmd;
    dbg_printf("output %d brightness %d\r\n",u8Port,pstBasicVar->u8Brightness[u8Port-1]);
    app_LMX_4IUHS_CPictureAdjust(u8Port-1,pstBasicVar->u8Brightness[u8Port-1],MS1826_ADJUST_BRIGHTNESS);
    SetUserData_SpecBasicVar();
}
...
void app_Type3CmdFunction(u8 u8Port,u8 u8Cmd)
{
    ...
    if(u8Cmd<=100)
        pstBasicVar->u8Contrast[u8Port-1] = u8Cmd;
    dbg_printf("output %d contrast %d\r\n",u8Port,pstBasicVar->u8Contrast[u8Port-1]);
    app_LMX_4IUHS_CPictureAdjust(u8Port-1,pstBasicVar->u8Contrast[u8Port-1],MS1826_ADJUST_CONTRAST);
    SetUserData_SpecBasicVar();
}
...
void app_Type4CmdFunction(u8 u8Port,u8 u8Cmd)
{
    ...
    if(u8Cmd<=100)
        pstBasicVar->u8Saturation[u8Port-1] = u8Cmd;
    dbg_printf("output %d saturation %d\r\n",u8Port,pstBasicVar->u8Saturation[u8Port-1]);
    app_LMX_4IUHS_CPictureAdjust(u8Port-1,pstBasicVar->u8Saturation[u8Port-1],MS1826_ADJUST_SATURATION);
    SetUserData_SpecBasicVar();
}
...
void app_Type5CmdFunction(u8 u8Port,u8 u8Cmd)
{
    ...
    if(u8Cmd<=100)
        pstBasicVar->u8Hue[u8Port-1] = u8Cmd;
    dbg_printf("output %d sharpness %d\r\n",u8Port,pstBasicVar->u8Hue[u8Port-1]);
    app_LMX_4IUHS_CPictureAdjust(u8Port-1,pstBasicVar->u8Hue[u8Port-1],MS1826_ADJUST_HUE);
    SetUserData_SpecBasicVar();
}
...
void app_Type6CmdFunction(pstUartCom_t pCommVar,u8 u8Port,u8 u8Cmd)
{
    ...
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    char l_s8ResText[20];
    u8 l_u8RealPort = cg_u8ChannelTable[u8Port-1][1]+1;
...
//            break;
        ...
        case 19:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1360x768_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1360x768_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
        ...
        case 20:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1920x1200_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1920x1200_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
        ...
        case 21:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1600x1200_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1600x1200_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 26:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1024x768_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1024x768_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 27:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1280x720_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1280x720_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 28:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1600x1200_60;
                app_MS1826TxResolutionChange(0,l_u16RealPort%4-1,emRes_1600x1200_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 29:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_1920x1080_60;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_1920x1080_60);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 30:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_3840x2160_30;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_3840x2160_30);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
         ...
         case 31:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8TxResolutionID[u8Port-1] = emRes_4096x2160_30;
                app_MS1826TxResolutionChange(0,l_u8RealPort%4-1,emRes_4096x2160_30);
                GetResolutionText(l_s8ResText,pstBasicVar->u8TxResolutionID[u8Port-1]);
                dbg_printf("out %d res %s\r\n",u8Port,l_s8ResText);
                SetUserData_SpecBasicVar();
            }
...
void app_Type7CmdFunction(pstUartCom_t pCommVar,u8 u8Port,u8 u8Cmd)
{
    ...
    switch(u8Cmd)
    {
        case 11:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8RxAudioSrc[u8Port-1] = 0;
                app_LMX_4IUHS_CAudioSourceSwitch(u8Port-1,pstBasicVar->u8RxAudioSrc[u8Port-1]);
                dbg_printf("out %d audio src %d\r\n",u8Port,pstBasicVar->u8RxAudioSrc[u8Port-1]);
                SetUserData_SpecBasicVar();
            }
            break;
        case 12:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8RxAudioSrc[u8Port-1] = 1;
                app_LMX_4IUHS_CAudioSourceSwitch(u8Port-1,pstBasicVar->u8RxAudioSrc[u8Port-1]);
                dbg_printf("out %d audio src %d\r\n",u8Port,pstBasicVar->u8RxAudioSrc[u8Port-1]);
                SetUserData_SpecBasicVar();
            }
...
void app_Type9CmdFunction(pstUartCom_t pCommVar,u8 u8Port,u8 u8Cmd)
{
    ...
    switch(u8Cmd)
    {
        case 0:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->bRxOsdStrEnable[u8Port-1] = FALSE;
                app_MS1826SetUserOsdStringEnable(0,u8Port-1,pstBasicVar->bRxOsdStrEnable[u8Port-1]);
                dbg_printf("in %d osd string disable\r\n",u8Port);
                SetUserData_SpecBasicVar();
            }
            break;
        case 1:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->bRxOsdStrEnable[u8Port-1] = TRUE;
                app_MS1826SetUserOsdStringEnable(0,u8Port-1,pstBasicVar->bRxOsdStrEnable[u8Port-1]);
                dbg_printf("in %d osd string enable\r\n",u8Port);
                SetUserData_SpecBasicVar();
            }
            break;
        case 2:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u16RxOsdStrX[u8Port-1] = 100;
                pstBasicVar->u16RxOsdStrY[u8Port-1] = 100;
                pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1] = FALSE;
                app_MS1826SetUserOsdStringPosition(0,u8Port-1,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
                app_MS1826SetOsdString(0,u8Port-1,pstBasicVar->u8RxOsdStrLen[u8Port-1],(u8 *)pstBasicVar->u8RxOsdStr[u8Port-1],pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1]);
                dbg_printf("in %d osd string posX:%d posY:%d\r\n",u8Port,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
                SetUserData_SpecBasicVar();
            }
            break;
        case 3:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u16RxOsdStrX[u8Port-1] = 1300;
                pstBasicVar->u16RxOsdStrY[u8Port-1] = 100;
                pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1] = TRUE;
                app_MS1826SetUserOsdStringPosition(0,u8Port-1,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
                app_MS1826SetOsdString(0,u8Port-1,pstBasicVar->u8RxOsdStrLen[u8Port-1],(u8 *)pstBasicVar->u8RxOsdStr[u8Port-1],pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1]);
                dbg_printf("in %d osd string posX:%d posY:%d\r\n",u8Port,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
                SetUserData_SpecBasicVar();
        ...
        case 4:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u16RxOsdStrX[u8Port-1] = 100;
                pstBasicVar->u16RxOsdStrY[u8Port-1] = 930;
                pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1] = FALSE;
                app_MS1826SetUserOsdStringPosition(0,u8Port-1,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
                app_MS1826SetOsdString(0,u8Port-1,pstBasicVar->u8RxOsdStrLen[u8Port-1],(u8 *)pstBasicVar->u8RxOsdStr[u8Port-1],pstBasicVar->bRxOsdStrIsAlignRight[u8Port-1]);
                dbg_printf("in %d osd string posX:%d posY:%d\r\n",u8Port,pstBasicVar->u16RxOsdStrX[u8Port-1],pstBasicVar->u16RxOsdStrY[u8Port-1]);
        ...
        case 6:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8RxOsdStrColorR[u8Port-1] = 0xFF;
                pstBasicVar->u8RxOsdStrColorG[u8Port-1] = 0xFF;
                pstBasicVar->u8RxOsdStrColorB[u8Port-1] = 0xFF;
                app_MS1826SetUserStringColor(0,u8Port-1,
                        pstBasicVar->u8RxOsdStrColorR[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorG[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorB[u8Port-1]);
                dbg_printf("in %d osd string color R:%d G:%d B:%d\r\n",u8Port,
                        pstBasicVar->u8RxOsdStrColorR[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorG[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorB[u8Port-1]);
                SetUserData_SpecBasicVar();
        ...
        case 7:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8RxOsdStrColorR[u8Port-1] = 0xFF;
                pstBasicVar->u8RxOsdStrColorG[u8Port-1] = 0x00;
                pstBasicVar->u8RxOsdStrColorB[u8Port-1] = 0x00;
                app_MS1826SetUserStringColor(0,u8Port-1,
                        pstBasicVar->u8RxOsdStrColorR[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorG[u8Port-1],
                        pstBasicVar->u8RxOsdStrColorB[u8Port-1]);
        ...
        case 8:
            {
                pCommVar->SendPrintf("OKEND");
                pstBasicVar->u8RxOsdStrColorR[u8Port-1] = 0x00;
                pstBasicVar->u8RxOsdStrColorG[u8Port-1] = 0xFF;
                pstBasicVar->u8RxOsdStrColorB[u8Port-1] = 0x00;
                app_MS1826SetUserStringColor(0,u8Port-1,
                        pstBasicVar->u8RxOsdStrColorR[u8Port-1],
...
void app_Type17CmdFunction(pstUartCom_t pCommVar,u8 u8Port,u8 u8Cmd)
{
    ...
    switch(u8Cmd)
    {
        ...
        case 14:
            ...
            break;
        case 28:
            pCommVar->SendPrintf("P%02d%02d%sEND",u8Port,u8Cmd,"B2301IAA0");
            dbg_printf("P%02d%02d%sEND",u8Port,u8Cmd,"B2301IAA0");
            break;
        default:
            break;
    }
        ...
        case 'A':    // full screen
            {
                ...
                {
                    //close splicer mode
                    app_LMX_4IUHS_CSetSplicerDisplay(u8Port,1,1,0);
        ...
        case 'B':    //2*2
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,2,2,l_u8Index);
            }
            break;
        case 'C':    //3*3
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,3,3,l_u8Index);
            }
            break;
        case 'D':    //4*4
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,4,4,l_u8Index);
            }
            break;
        case 'E':    //5*5
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,5,5,l_u8Index);
            }
            break;
        case 'F':    //6*6
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,6,6,l_u8Index);
            }
            break;
        case 'G':    //7*7
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,7,7,l_u8Index);
            }
            break;
        case 'H':    //8*8
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,8,8,l_u8Index);
            }
            break;
        case 'J':    //1*2
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,1,2,l_u8Index);
            }
            break;
        case 'K':    //1*3
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,1,3,l_u8Index);
            }
            break;
        case 'L':    //1*4
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,1,4,l_u8Index);
            }
            break;
        case 'M':    //2*3
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,2,3,l_u8Index);
            }
            break;
        case 'N':    //2*4
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,2,4,l_u8Index);
            }
            break;
        case 'O':    //3*4
            {
                ...
                app_LMX_4IUHS_CSetSplicerDisplay(u8Port,3,4,l_u8Index);
            }
            break;
        default:
            break;
...
static emCmdFuncStaType mapp_CmdfuncSetCardFunction(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    u8 l_u8CmdType = 0;
    u8 l_u8Cmd = 0;
    u16 l_u16CmdLen = pCommVar->u16RecCnt-strlen(cmd_list[cmdnum].ucmd);
    l_u8Port = app_GetPortSel()+1;
    if((l_pCmd[0]=='<')&&(l_pCmd[1]=='<')
        &&(l_pCmd[l_u16CmdLen-3]=='>')&&(l_pCmd[l_u16CmdLen-2]=='>')
        &&(l_pCmd[l_u16CmdLen-1]=='%'))
    {
        pstProjectSpecVar_t         pstSpecVar = &stLMX_4IUHS_CSpecVar;
        pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;

        pCommVar->SendPrintf("OKEND");
        app_MS1826SetOsdString(0,l_u8Port-1,l_u16CmdLen-5,(u8 *)(l_pCmd+2),pstBasicVar->bRxOsdStrIsAlignRight[l_u8Port-1]);
        memset(pstBasicVar->u8RxOsdStr[l_u8Port-1],' ',15);
        memcpy(pstBasicVar->u8RxOsdStr[l_u8Port-1], (u8 *)(l_pCmd+2),l_u16CmdLen-5);
        pstBasicVar->u8RxOsdStrLen[l_u8Port-1] = l_u16CmdLen-5;
        SetUserData_SpecBasicVar();
    }
    else
    {
        l_u8CmdType = HexAndAsciiChange(*(l_pCmd+0),0)*10+HexAndAsciiChange(*(l_pCmd+1),0);
        l_u8Cmd = HexAndAsciiChange(*(l_pCmd+2),0)*10+HexAndAsciiChange(*(l_pCmd+3),0);
        ...
        switch(l_u8CmdType)
        {
            ...
            case 2:
                pCommVar->SendPrintf("OKEND");
                app_Type2CmdFunction(l_u8Port,l_u8Cmd);
                break;
            case 3:
                pCommVar->SendPrintf("OKEND");
                app_Type3CmdFunction(l_u8Port,l_u8Cmd);
                break;
            case 4:
                pCommVar->SendPrintf("OKEND");
                app_Type4CmdFunction(l_u8Port,l_u8Cmd);
                break;
            case 5:
                pCommVar->SendPrintf("OKEND");
                app_Type5CmdFunction(l_u8Port,l_u8Cmd);
                break;
            case 6:
                {
                    app_Type6CmdFunction(pCommVar,l_u8Port,l_u8Cmd);
                }
                break;
            case 7:
                //pCommVar->SendPrintf("OKEND");
                app_Type7CmdFunction(pCommVar,l_u8Port,l_u8Cmd);
                break;
#if 0
            case 8:
                if(app_SwitchResolution((u8)l_u8Port,l_u8Cmd))
                {
                    pCommVar->SendPrintf("OKEND");
                }
                break;
#endif
            case 9:
                app_Type9CmdFunction(pCommVar,l_u8Port,l_u8Cmd);
                break;
            case 17:
                app_Type17CmdFunction(pCommVar,l_u8Port,l_u8Cmd);
                break;
            default:
                break;
        }
...
{
    emCmdFuncStaType l_state = emCmdNone;
    char *l_pCmd= pCommVar->pRecBuffer+strlen(cmd_list[cmdnum].ucmd)+2;
    u8 l_u8Port = 0;
    u8 l_u8CmdType = 0;
    u8 l_u8CmdVal = 0;
    u16 l_u16CmdLen = pCommVar->u16RecCnt-strlen(cmd_list[cmdnum].ucmd)-2;

    l_u8Port = pCommVar->pRecBuffer[strlen(cmd_list[cmdnum].ucmd)]-'0';
    l_u8Port = (l_u8Port-1)%4+1;

    if((l_pCmd[0]=='<')&&(l_pCmd[1]=='<')
        &&(((l_pCmd[l_u16CmdLen-3]=='>')&&(l_pCmd[l_u16CmdLen-2]=='>')&&(l_pCmd[l_u16CmdLen-1]=='%'))
        ||((l_pCmd[l_u16CmdLen-4]=='>')&&(l_pCmd[l_u16CmdLen-3]=='>')&&(l_pCmd[l_u16CmdLen-2]=='%'))))
    {
        pstProjectSpecVar_t         pstSpecVar = &stLMX_4IUHS_CSpecVar;
        pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    ...
    {
        l_u8CmdType = HexAndAsciiChange(*(l_pCmd+0),0)*10+HexAndAsciiChange(*(l_pCmd+1),0);
        l_u8CmdVal = HexAndAsciiChange(*(l_pCmd+2),0)*10+HexAndAsciiChange(*(l_pCmd+3),0);
        ...
        switch(l_u8CmdType)
        {
            ...
            case 1:
                pCommVar->SendPrintf("OKEND");
                app_Type1CmdFunction(l_u16Port,l_u8CmdVal);
                break;
#endif
            case 2:
                pCommVar->SendPrintf("OKEND");
                app_Type2CmdFunction(l_u8Port,l_u8CmdVal);
                break;
            case 3:
                pCommVar->SendPrintf("OKEND");
                app_Type3CmdFunction(l_u8Port,l_u8CmdVal);
                break;
            case 4:
                pCommVar->SendPrintf("OKEND");
                app_Type4CmdFunction(l_u8Port,l_u8CmdVal);
                break;
            case 5:
                pCommVar->SendPrintf("OKEND");
                app_Type5CmdFunction(l_u8Port,l_u8CmdVal);
                break;
            case 6:
                {
                    app_Type6CmdFunction(pCommVar,l_u8Port,l_u8CmdVal);
                }
                break;
            case 7:
                //pCommVar->SendPrintf("OKEND");
                app_Type7CmdFunction(pCommVar,l_u8Port,l_u8CmdVal);
                break;
#if 0
            case 8:
                if(app_SwitchResolution((u8)l_u8Port,l_u8CmdVal))
                {
                    pCommVar->SendPrintf("OKEND");
                }
                break;
#endif
            case 9:
                app_Type9CmdFunction(pCommVar,l_u8Port,l_u8CmdVal);
                break;
            case 17:
                app_Type17CmdFunction(pCommVar,l_u8Port,l_u8CmdVal);
                break;
            default:
                break;
        }
