...
{
    emKeyvalueTxKeyLeft       = BIT0,
    emKeyvalueTxKeyRignt      = BIT1,
    emKeyvalueRxKeyLeft       = BIT2,
    emKeyvalueRxKeyRignt      = BIT3,
}emKeyValueType;
...
{
    /*Group     Value               KeyNomalType
    ComboEnable  ComboTime     KeyComboType
    MultiFuncEnable MFConditionFlag   KeyMultiFuncType
    m_Flag*/
    {   0,      0,                  emKeyFunc_None,
        FALSE,  0,                  emKeyFunc_None,
        FALSE,  unUseMultiKeyFunc,  emKeyFunc_None,
        KeyFlag_NoUse},
    {   0,      emKeyvalueTxKeyLeft,emKeyFunc_TxIDHPlus,
        FALSE,  CtrlComboTime,      emKeyFunc_None,
        FALSE,  unUseMultiKeyFunc,  emKeyFunc_None,
        KeyFlag_Nomal
        },
    {   0,      emKeyvalueTxKeyRignt,emKeyFunc_TxIDLPlus,
        FALSE,  CtrlComboTime,      emKeyFunc_None,
        FALSE,  unUseMultiKeyFunc,  emKeyFunc_None,
        KeyFlag_Nomal
        },
    {   0,      emKeyvalueRxKeyLeft,emKeyFunc_RxIDHPlus,
        FALSE,  CtrlComboTime,      emKeyFunc_None,
        FALSE,  unUseMultiKeyFunc,  emKeyFunc_None,
        KeyFlag_Nomal
        },
    {   0,      emKeyvalueRxKeyRignt,emKeyFunc_RxIDLPlus,
        FALSE,  CtrlComboTime,      emKeyFunc_None,
        FALSE,  unUseMultiKeyFunc,  emKeyFunc_None,
        KeyFlag_Nomal
        },
...
{
    mapp_NewKeyScan();
    Project_STM32_SysFuncSel(mapp_GetKeyFunctionNum());
    mapp_EMX5RX_MainFunction();
}
#endif
...
void Project_STM32_SysFuncSel(iTE_u8 u8FuncKey)
{
    ...
    switch(u8FuncKey)
    {
        case emKeyFunc_TxIDHPlus:
            {
                u8 l_u8Temp = 0;
                if(pstBasicVar->u8TXID >= 100)
                {
                    pstBasicVar->u8TXID = 0;
                }
                l_u8Temp = (pstBasicVar->u8TXID)/10;
                l_u8Temp = (l_u8Temp+1)%10;
                pstBasicVar->u8TXID = l_u8Temp*10+(pstBasicVar->u8TXID%10);
                mapp_EMX5RX_SetInputID();
            }
            break;
        case emKeyFunc_TxIDLPlus:
            {
                u8 l_u8Temp = 0;
                if(pstBasicVar->u8TXID >= 100)
                {
                ...
                l_u8Temp = (pstBasicVar->u8TXID)%10;
                l_u8Temp = (l_u8Temp+1)%10;
                pstBasicVar->u8TXID = ((pstBasicVar->u8TXID)/10)*10+l_u8Temp;
                mapp_EMX5RX_SetInputID();
            }
            break;
        case emKeyFunc_RxIDHPlus:
            {
                u8 l_u8Temp = 0;
                if(pstBasicVar->u8RXID >= 100)
                {
                    pstBasicVar->u8RXID = 0;
                }
                l_u8Temp = (pstBasicVar->u8RXID)/10;
                l_u8Temp = (l_u8Temp+1)%10;
                pstBasicVar->u8RXID = l_u8Temp*10+(pstBasicVar->u8RXID%10);
                mapp_EMX5RX_SetLocalID();
            }
            break;
        case emKeyFunc_RxIDLPlus:
            {
                u8 l_u8Temp = 0;
                if(pstBasicVar->u8RXID >= 100)
                {
                ...
                l_u8Temp = (pstBasicVar->u8RXID)%10;
                l_u8Temp = (l_u8Temp+1)%10;
                pstBasicVar->u8RXID = ((pstBasicVar->u8RXID)/10)*10+l_u8Temp;
                mapp_EMX5RX_SetLocalID();
            }
            break;
