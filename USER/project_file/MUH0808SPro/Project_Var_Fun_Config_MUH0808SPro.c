...
stProjectSpecVar_t  stMUH0808SProSpecVar;
static volatile u32 g_MUH0808SPro_Timers[emUserTimer_Max];
stLT8644SwitchTable_T g_stMUH0808SProSwitchTable;
...
void app_MUH0808SProMS1826UserInit(void);
void app_MUH0808SProLT8644SwitchProcess(void);
void app_MUH0808SProMS1826UserProcess(void);
...
//------------------------------------------------------------------------------
void mapp_MUH0808SPro_PlatformTimerSet( uint8_t index, u32 m_sec )
{
    switch ( index )
    {
        default:
            ...
            {
                g_MUH0808SPro_Timers[index] = m_sec;
            }
            break;
    }
}
...
//------------------------------------------------------------------------------
u32 mapp_MUH0808SPro_PlatformTimerGet(uint8_t eTimer)
{
    return(g_MUH0808SPro_Timers[eTimer]);
}
...
// Description: 定时器初始化
//------------------------------------------------------------------------------
void mapp_MUH0808SPro_PlatformTimerInit (void)
{
    int i;
    //initializer timer counters in array
    for ( i = 0; i < emUserTimer_Max; i++ )
    {
        g_MUH0808SPro_Timers[ i ] = 0;
    }
...
// Description: 定时器递减函数
//------------------------------------------------------------------------------
void mapp_MUH0808SPro_PlatformTimerTask(void)
{
    int i;
    for(i=0;i<emUserTimer_Max;i++)
    {
        if(g_MUH0808SPro_Timers[i] > 0)
        {
            g_MUH0808SPro_Timers[i]--;
        }
    }
}
...
//------------------------------------------------------------------------------
uint8_t mapp_MUH0808SPro_PlatformTimerExpired ( uint8_t timer )           //定时时间到函�?
{
    if ( timer < emUserTimer_Max )
    {
        return( g_MUH0808SPro_Timers[ timer ] == 0);              //定时时间到时返回1
    }

    return( 0 );                                           //定时时间还没到时返回0
}
//========================================================================================
void MUH0808SPro_1msTimerIrq(void)
{
    mapp_MUH0808SPro_PlatformTimerTask();
}

void MUH0808SPro_SpecificVarInit(void)
{
...
void MUH0808SPro_SpecificVarDefault(void)
{
    MUH0808SPro_SpecBasicVarDefault();
    MUH0808SPro_SpecCustomEdidVarDefault();
}

void MUH0808SPro_SpecBasicVarDefault(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    u8 i;
...
void MUH0808SPro_SpecCustomEdidVarDefault(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
...
bool MUH0808SPro_SpecVarValidCheck(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
...
void MUH0808SPro_PrjStatePowerOnInit(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    ...
    // prj 1ms loop hook
    pFuncPrj1MsLoopHook = MUH0808SPro_1msTimerIrq;
    pvvFuncMS1826UserFunctionInit = app_MUH0808SProMS1826UserInit;
    pvvFuncMS1826UserFunctionProcess = app_MUH0808SProMS1826UserProcess;
    pLT8644Process = app_MUH0808SProLT8644SwitchProcess;
...

    {
        ...
        for(i=0;i<Def_outport_num;i++)
        {
            g_stMUH0808SProSwitchTable.u8OutCh[i] = pstPrjSaveVar->stHdmiSel.u8SwChannel[i];
            g_stMUH0808SProSwitchTable.bUpdateFlag[i] = TRUE;
            g_stMUH0808SProSwitchTable.bCloseEnableFlag[i] = FALSE;
        }
    }
    if(pstBasicVar->stScenePollingVar.bPollingEnable)
        mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_ScenePolling,40*1000);

    mapp_SetHdmiTx5V(TRUE);
    mapp_RevisionMake();
}

void MUH0808SPro_LedWork_Loop(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    static bool sl_bLedSta = FALSE;
    if(!mapp_MUH0808SPro_PlatformTimerExpired(emUserTimer_LedActive))
        return;
    mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_LedActive,500);
...
void MUH0808SPro_FuncWork_LoopRunning(void)
{
    MUH0808SPro_LedWork_Loop();
    app_MUH0808SProScenePollingFunction();
}

void mapp_MUH0808SProStartupMessage(void)
{
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
...
//        app_MS1826SetTxColorSpace(l_u8Dev,l_u8TxPort,0);
        ...
        app_MUH0808SProPictureAdjust(l_u8Dev,l_u8TxPort,pstBasicVar->u8Brightness[i],MS1826_ADJUST_BRIGHTNESS);
        app_MUH0808SProPictureAdjust(l_u8Dev,l_u8TxPort,pstBasicVar->u8Contrast[i],MS1826_ADJUST_CONTRAST);
        app_MUH0808SProPictureAdjust(l_u8Dev,l_u8TxPort,pstBasicVar->u8Saturation[i],MS1826_ADJUST_SATURATION);
        app_MUH0808SProPictureAdjust(l_u8Dev,l_u8TxPort,pstBasicVar->u8Hue[i],MS1826_ADJUST_HUE);
#if 0
        ...
        app_MS1826TxDigitalAudioSourceSelect(l_u8Dev,l_u8TxPort,cg_u8ChannelTableTX[i][2]);

        app_MUH0808SProSetSplicerPixelDisplay(l_u8Dev,l_u8TxPort,
    ...
    mapp_SetAudioMuteByPCA9539(0,FALSE);
    mapp_SetAudioMuteByPCA9539(1,FALSE);
    mapp_SetAudioMuteByPCA9539(2,FALSE);
    mapp_SetAudioMuteByPCA9539(3,FALSE);
    mapp_SetAudioMuteByPCA9539(4,FALSE);
    mapp_SetAudioMuteByPCA9539(5,FALSE);
    mapp_SetAudioMuteByPCA9539(6,FALSE);
    mapp_SetAudioMuteByPCA9539(7,FALSE);
}
...
void app_MUH0808SProMS1826UserProcess(void)
{
    ...
    for(i=0;i<Def_outport_num;i++)
    {
        ...
        if(g_stMUH0808SProSwitchTable.bUpdateFlag[i])
        {
            if(l_u8Dev==2)
    ...
    if(l_bTxFreezeFlag)
    {
//        app_MUH0808SProSwitchSourceFreeze(2,l_u8TxPath1&0x0F,TRUE);
//        app_MUH0808SProSwitchSourceFreeze(3,l_u8TxPath2&0x0F,TRUE);
...
//                app_MUH0808SProSwitchSourceFreeze(2,0x0F,FALSE);
//                app_MUH0808SProSwitchSourceFreeze(3,0x0F,FALSE);
                l_u8TxPath[0]=0x0F;
                l_u8TxPath[1]=0x0F;
...
void app_MUH0808SProSetSplicerPixelDisplay(u8 u8Dev,u8 u8Tx,u16 u16X,u16 u16Y,u16 u16Width,u16 u16Height)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
...
void app_MUH0808SProSetSplicerDisplay(u8 u8Dev,u8 u8Tx,u8 u8H,u8 u8W,u8 u8Index)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
//    stTimingTable_t l_stRxTming;
    ...
    app_MUH0808SProSetSplicerPixelDisplay(u8Dev,u8Tx,l_u16X,l_u16Y,l_u16W,l_u16H);

    SetUserData_SpecBasicVar();
}

void app_MUH0808SProSetSplicerBorder(u8 u8Tx,u8 u8H,u8 u8W,u8 u8Index,u16 u16BorderWidth,u16 u16BorderHeight)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
...
void app_MUH0808SProPictureAdjust(u8 u8Dev,u8 u8Tx,u8 u8Value,u8 u8Mode)
{
    app_MS1826PictureAdjust(u8Dev,u8Tx,u8Value+78,u8Mode);
}

void app_MUH0808SProSourceSwitch(u8 u8Out,u8 u8In)
{
//    u8 l_u8Dev;

//    l_u8Dev  = (u8)cg_u8ChannelTableTX[u8Out][3];

    g_stMUH0808SProSwitchTable.u8OutCh[u8Out] = u8In;
    g_stMUH0808SProSwitchTable.bUpdateFlag[u8Out] = TRUE;
    g_stMUH0808SProSwitchTable.bCloseEnableFlag[u8Out] = FALSE;
...
//        l_u8TxPort = (u8)cg_u8ChannelTableTX[i][0];
//        l_u8Dev  =   (u8)cg_u8ChannelTableTX[i][3];
        if((TRUE == g_stMUH0808SProSwitchTable.bUpdateFlag[i])&&(g_u8SrcChgTxFreezeState==1))
        {
            in=Ka_ch_adjust_in[g_stMUH0808SProSwitchTable.u8OutCh[i]*2];
            ch = Ka_ch_adjust_out[i*2];
//            dbg_printf("In[%d] Out[%d]\r\n",in,ch);
            if(g_stMUH0808SProSwitchTable.bCloseEnableFlag[i] == FALSE)
            {
                Data =  ((ch_in[in+1])<<4) ;
                Data |= ch_in[in];
            ...
            g_stMUH0808SProSwitchTable.bUpdateFlag[i] = FALSE;
        }
    }
...
void app_MUH0808SProSceneSave(u8 u8SceneID)
{
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
...
void app_MUH0808SProSceneCall(u8 u8SceneID)
{
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    ...
    if(u8SceneID<32)
    {
        if(pstBasicVar->stScene[u8SceneID].u8SceneSaveFlag == TRUE)
        {
            ...
            {
                // switch source
                app_MUH0808SProSourceSwitch(i,pstPrjSaveVar->stHdmiSel.u8SwChannel[i]);
...
// 场景轮询;
void app_MUH0808SProScenePollingFunction(void)
{
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    ...
    if(pstBasicVar->stScenePollingVar.bPollingEnable)
    {
        ...
        if(!mapp_MUH0808SPro_PlatformTimerExpired(emUserTimer_ScenePolling))
            return;
        mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_ScenePolling,pstBasicVar->stScenePollingVar.u16PollingTime*1000);
        memset(l_bSwSplicerFlag,FALSE,Def_outport_num);
...
/******************************************************
///Function Name:   app_MUH0808SProReadEdidDescriptor1Timing
...
///Others:          NULL
******************************************************/
void app_MUH0808SProReadEdidDescriptor1Timing(pu8 pu8EdidBuf,pstTimingTable_t pstEdidRes)
{
    u16 l_u16HBlank;
    u16 l_u16VBlank;
...
/***********************************************************************
 * FunName : app_MUH0808SProAutoResolutionConfig
 * Writer  : wf8421
 ...
 * History :
 ***********************************************************************/
void app_MUH0808SProAutoResolutionConfig(u8 u8Tx)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    ...
    if(pstBasicVar->stAutoTimingTable[u8Tx].bEnable)
    {
        ...
        if(l_bEdidSta)
        {
            app_MUH0808SProReadEdidDescriptor1Timing(l_u8EdidBuf,&l_stTiming);
...
/***********************************************************************
 * FunName : app_MUH0808SProAutoResolutionProcess
 * Writer  : wf8421
 ...
 * History :
 ***********************************************************************/
void app_MUH0808SProAutoResolutionProcess(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stMUH0808SProSpecVar;
    pstBasicSpecVar_t           pstBasicVar = &pstSpecVar->stBasSpecVar;
    u8 i;
    ...
    for(i=0;i<Def_outport_num;i++)
    {
        if(pstBasicVar->stAutoTimingTable[i].bEnable)
        {
            if(g_stLinkSta.u8OutLinkSta[i]!=sl_u8PreLinkSta[i])
            {
                ...
                app_MUH0808SProAutoResolutionConfig(i);
            }
        }
    }
}
...
//    mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_ReportTiming,10000);
//    mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_TimingChange,10000);
    g_stUpdateVar.u8UpdateMode = u8Id;
    switch(u8Step)
    {
        case 0:
            {
                mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_FirmwareUpdate,10000);
                g_stUpdateVar.u8UpdateFlag=1;
                //if(u8Id==eUpdID_MCU)
                {
...
u8 mapp_FirmwareUpdateProcess(pstUartCom_t pComVar)
{
    ...
    {
        mapp_MUH0808SPro_PlatformTimerSet(emUserTimer_FirmwareUpdate,2000);
...
{
    if(g_stUpdateVar.u8UpdateFlag==1&&mapp_MUH0808SPro_PlatformTimerExpired(emUserTimer_FirmwareUpdate))
    {
