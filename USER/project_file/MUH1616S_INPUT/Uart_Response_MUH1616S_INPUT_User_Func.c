...
#include "../USER/project_file/MUH1616S_INPUT/mapp_UartCmdList_MUH1616S_INPUT.h"
...
u8 g_u8OutputFwversion;

bool g_bEdidCopyTxFlag = FALSE;
u8 g_u8EdidCopyToRxIndex = 0;
...
{
    pCommVar->SendPrintf("mcu rx %s\r\n",cmd_list[cmdnum].feedbackstring);
    pCommVar->SendPrintf("mcu tx v%d.%d.%d\r\n",(g_u8OutputFwversion>>5)&0x07,(g_u8OutputFwversion>>3)&0x02,(g_u8OutputFwversion)&0x07);
...
static emCmdFuncStaType mapp_CmdfuncFactoryReset(pstUartCom_t pCommVar,u8 cmdnum)
{
    u8 l_u8Value[2];
    l_u8Value[0] = 01;
    mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_ResetFactory,l_u8Value,1);
...
static emCmdFuncStaType mapp_CmdfuncReboot(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    l_u8Value[0] = 01;
    mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_RebootSystem,l_u8Value,1);
    delay_ms(100);
    SYS_SoftReset();
...
static emCmdFuncStaType mapp_CmdfuncSetTxResolution(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8ResID>=1&&l_s8ResID<=emRes_max))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = l_s8ResID;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SendResID,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetTxHdcpMode(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8HdcpMode>=0&&l_s8HdcpMode<=1))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = l_s8HdcpMode;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetHdcp,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetTxFormat(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8TxFormat>=0&&l_s8TxFormat<=1))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = l_s8TxFormat;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetFormat,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetFreezeState(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8FreeSta>=0&&l_s8FreeSta<=1))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = l_s8FreeSta;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetOutPortFreeze,l_u8Value,2);
        //SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetBrightness(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Brightness<=BRIGHTNESS_VALUE_MAX))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = (u8)l_s16Brightness;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetBritness,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetContrast(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=CONTRAST_VALUE_MAX))
    {
        ...
        l_u8Value[0] = l_s8Output;
        l_u8Value[1] = (u8)l_s16Value;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetContrast,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=SATURATION_VALUE_MAX))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                app_MUH1616S_INPUTPictureAdjust(l_u8Dev,l_u8RealPort,(u8)l_s16Value,MS1826_ADJUST_SATURATION);
            }
        }
        else
        {
            ...
            pCommVar->SendPrintf("output %d saturation %d\r\n",l_s8Output,l_s16Value);
            l_u8Dev = cg_u8ChannelTable[l_s8Output-1][3];
            l_u8RealPort = cg_u8ChannelTable[l_s8Output-1][0];
            app_MUH1616S_INPUTPictureAdjust(l_u8Dev,l_u8RealPort,(u8)l_s16Value,MS1826_ADJUST_SATURATION);
        }
        SetUserData_SpecBasicVar();
    }
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s16Value<=HUE_VALUE_MAX))
    {
        if(l_s8Output == 0)
        {
            ...
            for(i=0;i<Def_outport_num;i++)
            {
                ...
                l_u8Dev = cg_u8ChannelTable[i][3];
                l_u8RealPort = cg_u8ChannelTable[i][0];
                app_MUH1616S_INPUTPictureAdjust(l_u8Dev,l_u8RealPort,(u8)l_s16Value,MS1826_ADJUST_HUE);
            }
        }
        else
        {
    ...
    if(l_s8EdidID>0&&l_s8EdidID<EDID_MODE_MAX&&l_s8Input<=_HDMI_INPUT_PORT_USED_NUMBER)
    {
        switch(l_s8EdidID)
        {
            ...
            case EDID_MODE_COPYTX16:
                {
                    u8 l_u8Value[2];
                    l_u8Value[0] = l_s8EdidID-EDID_MODE_COPYTX1+1;
                    g_bEdidCopyTxFlag = TRUE;
                    g_u8EdidCopyToRxIndex = l_s8Input;
                    mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_GetTxPortEdid,l_u8Value,1);
                }
                break;
            default:
                break;
...
static emCmdFuncStaType mapp_CmdfuncSetTxTiming(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    {
        l_u8Value[0] = l_s8Output;
        memcpy(&l_u8Value[1],&l_stTiming,sizeof(stTimingTable_t));
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_GetTxPortEdid,l_u8Value,28);
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetTxPattern(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8TxPatternID<=17))
    {
        ...
        {
            pstBasicVar->u8PatternSta[l_u8RealPort-1] = l_s8TxPatternID;
            pCommVar->SendPrintf("output %d pattern is %d\r\n",l_u8RealPort,l_s8TxPatternID);
        }
        l_u8Value[0] = l_u8RealPort;
        l_u8Value[1] = (u8)l_s8TxPatternID;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetOutputPattern,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else if((l_s8Output<=(Def_outport_num+_HDMI_INPUT_PORT_USED_NUMBER+1))&&(l_s8TxPatternID<=17))
    {
        u8 l_u8Input = l_s8Output-Def_outport_num;
        if(l_u8Input == (_HDMI_INPUT_PORT_USED_NUMBER+1))
        {
            u8 i;
            for(i=0;i<_HDMI_INPUT_PORT_USED_NUMBER;i++)
            {
                ...
                l_u8Dev = cg_u8ChannelTable[i][3];
                pstBasicVar->u8InputPatternSta[i] = l_s8TxPatternID;
                pCommVar->SendPrintf("input %d pattern is %d\r\n",i+1,l_s8TxPatternID);
                if(pstBasicVar->u8InputPatternSta[i])
                    app_MS1826TxPatternConfig(l_u8Dev,l_u8RealPort-1,pstBasicVar->u8InputPatternSta[i]-1,1);
                else
        ...
        {
            l_u8RealPort = cg_u8ChannelTable[l_u8Input-1][1]+1;
            l_u8Dev = cg_u8ChannelTable[l_u8Input-1][3];
            pstBasicVar->u8InputPatternSta[l_u8Input-1] = l_s8TxPatternID;
            pCommVar->SendPrintf("input %d pattern is %d\r\n",l_u8Input,l_s8TxPatternID);
            if(pstBasicVar->u8InputPatternSta[l_u8Input-1])
                app_MS1826TxPatternConfig(l_u8Dev,l_u8RealPort-1,pstBasicVar->u8InputPatternSta[l_u8Input-1]-1,1);
            else
                app_MS1826TxPatternConfig(l_u8Dev,l_u8RealPort-1,0,0);
        }
        SetUserData_SpecBasicVar();
...
static emCmdFuncStaType mapp_CmdfuncGetTxPattern(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    else if(l_s8Output<=(Def_outport_num+_HDMI_INPUT_PORT_USED_NUMBER+1))
    {
        u8 l_u8Input = l_s8Output-_HDMI_INPUT_PORT_USED_NUMBER;
        if(l_u8Input == (_HDMI_INPUT_PORT_USED_NUMBER+1))
        {
            u8 i;
            for(i=0;i<_HDMI_INPUT_PORT_USED_NUMBER;i++)
            {
                pCommVar->SendPrintf("input %d pattern is %d\r\n",i+1,pstBasicVar->u8InputPatternSta[i]);
            }
        }
        else
        {
            pCommVar->SendPrintf("input %d pattern is %d\r\n",l_s8Output,pstBasicVar->u8InputPatternSta[l_u8Input-1]);
        }
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSetSplitConfig(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    char l_s8Port;
    u8 l_u8NumLen=0;
    u8 l_u8RealPort,u8Enalbe,u8Dev;
    ...
    if(l_s8Port == 0)
    {
        ...
        {
            l_u8RealPort = cg_u8ChannelTable[i][0]+1;
            u8Dev = cg_u8ChannelTable[i][3];
            app_MS1826SplitConfig(u8Dev,l_u8RealPort-1,u8Enalbe,l_u16X,l_u16Y,l_u16W,l_u16H);
    ...
    else if(l_s8Port<=_HDMI_INPUT_PORT_USED_NUMBER)
    {
        ...
        u8Dev = cg_u8ChannelTable[l_s8Port-1][3];
        app_MS1826SplitConfig(u8Dev,l_u8RealPort-1,u8Enalbe,l_u16X,l_u16Y,l_u16W,l_u16H);
...
static emCmdFuncStaType mapp_CmdfuncSetSplicerConfig(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(l_s8Port == 0)
    {
        ...
        l_u8Value[0] = 0;
        l_u8Value[1] = u8Enalbe;
        l_u8Value[2] = l_u16X&0xFF;
        l_u8Value[3] = (l_u16X>>8)&0xFF;
        l_u8Value[4] = l_u16Y&0xFF;
        l_u8Value[5] = (l_u16Y>>8)&0xFF;
        l_u8Value[6] = l_u16W&0xFF;
        l_u8Value[7] = (l_u16W>>8)&0xFF;
        l_u8Value[8] = l_u16H&0xFF;
        l_u8Value[9] = (l_u16H>>8)&0xFF;
...
static emCmdFuncStaType mapp_CmdfuncSetSplicerPosition(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Group<=2&&l_s8Group>0)&&(l_u8Port<=Def_outport_num&&l_u8Port>0))
    {
        ...
        if((l_u8X>0&&l_u8X<=l_u8W)&&(l_u8Y>0&&l_u8Y<=l_u8H))
        {
            ...
            app_MUH1616S_INPUTSetSplicerBorder(l_u8Port-1,l_u8H,l_u8W,l_u8Index,
                        pstBasicVar->stSplicerGroup[l_s8Group-1].u8BorderSizeWidth,pstBasicVar->stSplicerGroup[l_s8Group-1].u8BorderSizeHeight);
            app_MUH1616S_INPUTSetSplicerDisplay(l_u8Dev,l_u8Port-1,l_u8H,l_u8W,l_u8Index);
            ...
            pCommVar->SendPrintf("splicer position:\r\n"
                                 ...
                                 "    port: %d,\r\n"
                                 "    ypos: %d,\r\n"
                                 "    xpos: %d;\r\n",l_s8Group,l_u8Port,l_u8Y,l_u8X);
        }
        else
            l_state = emCmdOutOfRange;
    }
    ...
    if(l_s8Group == 0)
    {
        for(i=0;i<Def_outport_num;i++)
        {
            if(pstBasicVar->stSplicerGroup[0].bGroupTxEnable[i] == TRUE)
            {
                ...
                pstBasicVar->stSplicerGroup[0].u8SplicerPosX[i] = 0;
                pstBasicVar->stSplicerGroup[0].u8SplicerPosY[i] = 0;
                pstBasicVar->bSplicerEnable[i] = FALSE;

                l_u8Value[0] = i+1;
                l_u8Value[1] = (u8)pstBasicVar->bSplicerEnable[i];
                mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_TxWinClear,l_u8Value,2);
            }
            if(pstBasicVar->stSplicerGroup[1].bGroupTxEnable[i] == TRUE)
            {
                pstBasicVar->stSplicerGroup[1].bGroupTxEnable[i] = FALSE;
                pstBasicVar->stSplicerGroup[1].u8SplicerPosX[i] = 0;
                pstBasicVar->stSplicerGroup[1].u8SplicerPosY[i] = 0;
                pstBasicVar->bSplicerEnable[i] = FALSE;
    ...
    else if(l_s8Group<=Def_outport_num)
    {
        for(i=0;i<Def_outport_num;i++)
        {
            if(pstBasicVar->stSplicerGroup[l_s8Group-1].bGroupTxEnable[i] == TRUE)
            {
                ...
                pstBasicVar->bSplicerEnable[i]   = FALSE;
                pstBasicVar->stSplicerGroup[l_s8Group-1].u8SplicerPosX[i] = 0;
                pstBasicVar->stSplicerGroup[l_s8Group-1].u8SplicerPosY[i] = 0;
...
static emCmdFuncStaType mapp_CmdfuncModeSwitch(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8WorkMode<=4)&&(l_s8ChipIndex>=1&&l_s8ChipIndex<=4))
    {
...
static emCmdFuncStaType mapp_CmdfuncSetTxExtAudio(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8Output<=Def_outport_num)&&(l_s8ADOn>=0&&l_s8ADOn<=1))
    {
        ...
        l_u8Value[0] = l_u8RealPort;
        l_u8Value[1] = (u8)l_s8ADOn?0:1;
        mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_SetDigitalAudioMute,l_u8Value,2);
        SetUserData_SpecBasicVar();
    }
    else
        l_state = emCmdOutOfRange;
...
static emCmdFuncStaType mapp_CmdfuncSaveScene(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    {
        app_MUH1616S_INPUTSceneSave(l_s8SceneID-1);
...
static emCmdFuncStaType mapp_CmdfuncCallScene(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if(l_s8SceneID<=32&&l_s8SceneID>=1)
    {
        ...
        {
            app_MUH1616S_INPUTSceneCall(l_s8SceneID-1);
...
static emCmdFuncStaType mapp_CmdfuncSetSplitMode(pstUartCom_t pCommVar,u8 cmdnum)
{
        ...
        &&(l_u8Mode<=16))
    {
        if(l_u8Group==0)
        {
            for(i=0;i<4;i++)
            {
                app_MS1826SetSplitterMode(i,l_u8Mode);
        ...
        {
            app_MS1826SetSplitterMode(l_u8Group-1,l_u8Mode);
    ...
    if(((l_s8Group<=4)&&(l_s8Group>0)))
    {
        if(l_s8Group == 0)
        {
            ...
            pCommVar->SendPrintf("group %d splitter video map:\r\n"
                                 "A B C D\r\n"
                                 "%d %d %d %d;\r\n",3,
                                 pstBasicVar->u8SplitterWinSrc[2][0]+1,
                                 pstBasicVar->u8SplitterWinSrc[2][1]+1,
                                 pstBasicVar->u8SplitterWinSrc[2][2]+1,
                                 pstBasicVar->u8SplitterWinSrc[2][3]+1);
            pCommVar->SendPrintf("group %d splitter video map:\r\n"
                                 "A B C D\r\n"
                                 "%d %d %d %d;\r\n",4,
                                 pstBasicVar->u8SplitterWinSrc[3][0]+1,
                                 pstBasicVar->u8SplitterWinSrc[3][1]+1,
                                 pstBasicVar->u8SplitterWinSrc[3][2]+1,
                                 pstBasicVar->u8SplitterWinSrc[3][3]+1);
        }
        else
        {
            pCommVar->SendPrintf("group %d splitter video map:\r\n"
...
void mapp_SetPowerState(bool bPwrSta)
{
    ...
    if(bPwrSta)
    {
        if(!pstBasicSpecVar->bPowerOnSta)
        {
            ...
            l_u8Value[0] = 1;
            mapp_MUH1616S_InputSendCmdToOutput(eMcuReg_PowerState,l_u8Value,1);
        }
    }
    else
    {
...
//    dbg_printf("Tx link sta 0x%04x\r\n",l_u16TxLinkSta);
    ...
    for(i=0;i<Def_outport_num;i++)
    {
        ...
        if(l_u8OutLinkSta[i]!=g_stLinkSta.u8OutLinkSta[i])
        {
            ...
            g_stLinkSta.bOutLinkChangeFlag[i] = TRUE;
        }
    }
    return (u8)l_emRtState;
}
//eMcuReg_HdcpAndFormat       = 0x04,
u8 mapp_TxFuncSetFormat(char *pCmdBuf,u16 u16CmdLen)
{
    emCmdFuncStaType  l_emRtState = emCmdSucess;
...
//    u8 l_u8edidbuf[256];
    ...
    if(l_u8TxPort>0&&l_u8TxPort<=Def_outport_num)
    {
        ...
        if(g_bEdidCopyTxFlag)
        {
            u8 l_u8RealPort,l_u8Dev;

            l_u8RealPort = cg_u8ChannelTable[g_u8EdidCopyToRxIndex-1][0]+1;
            l_u8Dev = cg_u8ChannelTable[g_u8EdidCopyToRxIndex-1][3];
            if(is_edid_valid((iTE_u8 *)&pCmdBuf[8]))
            {
                app_MS1826RxEdidUpdate(l_u8Dev,l_u8RealPort-1,(u8 *)&pCmdBuf[8],256);
            }
            g_bEdidCopyTxFlag = FALSE;
			dbg_printf("output %d edid copy to input %d\r\n",l_u8TxPort,g_u8EdidCopyToRxIndex);
        }
    }
    return (u8)l_emRtState;
}
...
u8 mapp_TxFuncSetContrast(char *pCmdBuf,u16 u16CmdLen)
{
    emCmdFuncStaType  l_emRtState = emCmdSucess;
...
u8 mapp_TxFuncSetHdcp(char *pCmdBuf,u16 u16CmdLen)
{
    emCmdFuncStaType  l_emRtState = emCmdSucess;
...
u8 mapp_TxFuncSetOutPortFreeze(char *pCmdBuf,u16 u16CmdLen)
{
    emCmdFuncStaType  l_emRtState = emCmdSucess;
...
//eMcuReg_MsgFreezeOk
u8 mapp_TxFuncMsgFreezeOk(char *pCmdBuf,u16 u16CmdLen)
{
    emCmdFuncStaType  l_emRtState = emCmdSucess;
    g_bMessageFreezeOkFlag = TRUE;
    dbg_printf("out board freeze ok\r\n");
    return (u8)l_emRtState;
}

stUartList_t Uart4TxCmdTable[]=
{
    {eMcuReg_CardType,      mapp_TxFuncCardType},
    {eMcuReg_Version,       mapp_TxFuncGetVersion},
    {eMcuReg_LinkSta,       mapp_TxFuncLinkSta},
    {eMcuReg_SetFormat,     mapp_TxFuncSetFormat},
    ...
    {eMcuReg_SetContrast,   mapp_TxFuncSetContrast},
    {eMcuReg_SetHdcp,       mapp_TxFuncSetHdcp},
    {eMcuReg_SendResID,     mapp_TxFuncSendResID},
    {eMcuReg_SetOutPortFreeze,  mapp_TxFuncSetOutPortFreeze},
    {eMcuReg_MsgFreezeOk,   mapp_TxFuncMsgFreezeOk},

    //end
    {0,NULL},
};

void mapp_Usart4SystemCmdFun(pstUartCom_t pComVar)
{
    emCmdFuncStaType l_bStatus=emCmdError;
    u8 l_u8Addr;
    u8 l_u8Cmd;
                ...
                case 0x81:
                    {
                        for(i=0;i<=255;i++)
                        {
                            if((Uart4TxCmdTable[i].u8Cmd == 0)||(Uart4TxCmdTable[i].pCmdFun == NULL))
                                break;
                            if((l_u8Cmd == Uart4TxCmdTable[i].u8Cmd)&&(Uart4TxCmdTable[i].pCmdFun!=NULL))
                            {
                                l_bStatus = (emCmdFuncStaType)Uart4TxCmdTable[i].pCmdFun(l_ps8CmdBuf,(l_u16CmdLen+7));
                                if(l_bStatus == emCmdError)
                                {
                                }
                                break;
