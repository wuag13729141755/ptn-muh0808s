...
uint8_t g_u8Input10PlusCnt = 0;
uint8_t g_u8Output10PlusCnt = 0;
...
static stKeyScanType stKeyScanGroup[]=
{
    ...
    {  0,   emKeyvalueInput9,       emKeyFunc_In9,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
    {  0,  emKeyvalueInput10,      emKeyFunc_In10,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
    {  0,  emKeyvalueInput11,  emKeyFunc_In10Plus,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
    ...
    {  0,  emKeyvalueOutput9,      emKeyFunc_Out9,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
    {  0, emKeyvalueOutput10,     emKeyFunc_Out10,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
    {  0, emKeyvalueOutput11, emKeyFunc_Out10Plus,          FALSE,   CtrlComboTime,   emKeyFunc_None,    FALSE,   unUseMultiKeyFunc,   emKeyFunc_None,  KeyFlag_Nomal},
...
void mapp_KeyFunctionProcess(void)
{
    ...
    MUH1616S_INPUT_FuncWork_LoopRunning();
}
...
//    portENTER_CRITICAL();
	...
	switch(u8FuncKey)
	{
        ...
        case emKeyFunc_In9:
        case emKeyFunc_In10:
            OperateStatsu(u8FuncKey-emKeyFunc_In1+1+10*g_u8Input10PlusCnt);
			break;
        case emKeyFunc_In10Plus:
            OperateStatsu(IN10PLUS);
            break;
        ...
        case emKeyFunc_Out8:
        case emKeyFunc_Out9:
        case emKeyFunc_Out10:
            OperateStatsu(u8FuncKey-emKeyFunc_Out1+IN_MAX+1+10*g_u8Output10PlusCnt);
            break;
        case emKeyFunc_Out10Plus:
            OperateStatsu(OUT10PLUS);
            break;
        ...
        case emKeyFunc_CheckOutCh9:
        case emKeyFunc_CheckOutCh10:
            OperateStatsu(CHCEK_OUT_STA+u8FuncKey-emKeyFunc_CheckOutCh1);
            break;
        case emKeyFunc_CheckOutCh10Plus:
            OperateStatsu(CHCEKOUT10PLUS);
            break;
#endif
		default:
			break;
...
{
    mapp_MUH1616S_INPUT_PlatformTimerSet(emUserTimer_LcdBacklightTimeout,15000);
...
void mapp_LCDBacklightMuteProcess(void)
{
    ...
    {
        if(mapp_MUH1616S_INPUT_PlatformTimerExpired(emUserTimer_LcdBacklightTimeout))
        {
...
void InputStatusHandle(void)
{
    ...
    if(IN_channel>_HDMI_INPUT_PORT_USED_NUMBER)
    {
        KeyStatus = ErrorUseStatus;
        return;
    }

    mapp_SetLedState(led_name_LedInput1+IN_channel-1-10*g_u8Input10PlusCnt,TRUE);
    if(g_u8Input10PlusCnt)
        mapp_SetLedState(led_name_LedInput10Plus,TRUE);
...
void FirstOutputStaHandle(void)
{
    ...
    if(KeyBuffValue.OutputKey[0]>Def_outport_num)
    {
        KeyStatus = ErrorUseStatus;
        return;
    }

    mapp_SetLedState(led_name_LedOutput1+KeyBuffValue.OutputKey[0]-1-10*g_u8Output10PlusCnt,TRUE);
    if(g_u8Output10PlusCnt)
        mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
    LED_RecoverToDefault(DEF_LED_DISPLAY_TIME);
...
void OutputStatusHandle(void)
{
	...
	outputkeytempbuff = KeyBuffValue.OutputKey[KeyBuffValue.OutputCnt - 1]-IN_MAX;

    if(outputkeytempbuff>Def_outport_num)
    {
        KeyStatus = ErrorUseStatus;
        return;
    }
    mapp_SetLedState(led_name_LedOutput1+outputkeytempbuff-1-10*g_u8Output10PlusCnt,TRUE);
    if(g_u8Output10PlusCnt)
        mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
	strncpy(l_cDisplayText, sys_type,16);   // LCD 屏幕显示内容
	LCD_LineRefresh(l_cDisplayText, LINE1);
...
void AllOutKeyStaHandle(void)
{
    ...
    char l_cDisplayText[16];
//    TickType_t u32StarTime = xTaskGetTickCount();

    for(CNT = led_name_LedOutput1; CNT<=led_name_LedOutput10Plus;CNT++)
    ...
    for(i=0;i<Def_outport_num;i++)
    {
        ...
        if(IN_channel>0)
        {
            app_MUH1616S_INPUTSourceSwitch(i,IN_channel-1);
    ...
    USER_printf("ok\r\n");
    SetUserData_HdmiSel();
//    xTaskDelayUntil(&u32StarTime,50);
	strncpy(l_cDisplayText, sys_type,16);   // LCD 屏幕显示内容
	LCD_LineRefresh(l_cDisplayText, LINE1);
    ...
    g_u8Input10PlusCnt = 0;
    g_u8Output10PlusCnt = 0;
}
...
void EnterCheckChannleHandle(void)
{
    ...
    {
        mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
        mapp_SetLedState(led_name_LedOutput1+OUT_num-1-10,TRUE);
    }
    else
        mapp_SetLedState(led_name_LedOutput1+OUT_num-1,TRUE);

    if(pstPrjSaveVar->stHdmiSel.u8SwChannel[OUT_num-1]>10)
    {
        mapp_SetLedState(led_name_LedInput10Plus,TRUE);
        mapp_SetLedState(led_name_LedInput1+pstPrjSaveVar->stHdmiSel.u8SwChannel[OUT_num-1]-10,TRUE);
    }
    else
    {
    ...
    i = pstPrjSaveVar->stHdmiSel.u8SwChannel[OUT_num-1]+1;
    strncpy(l_cDisplayText, "Out 01 In       ",16);
    l_cDisplayText[4] =	OUT_num/10+0x30;
    l_cDisplayText[5] = OUT_num%10+0x30;
    l_cDisplayText[10] = i/10+'0';
    l_cDisplayText[11] = i%10+'0';
    LCD_LineRefresh(l_cDisplayText,LINE3);
...
void EnterSwitchStatusHandle(void)
{
    ...
    {
    	OUT_channel[0] = KeyBuffValue.OutputKey[0] - IN_MAX;

        if (OUT_channel[0] > 0)
        {
            app_MUH1616S_INPUTSourceSwitch(OUT_channel[0]-1,IN_channel-1);
    ...
    else
    {
        ...
        {
            OUT_channel[i] = KeyBuffValue.OutputKey[i] - IN_MAX;

            if (OUT_channel[i] > 0)
            {
                app_MUH1616S_INPUTSourceSwitch(OUT_channel[i]-1,IN_channel-1);
...
void SceneIdxInputStatusHandle(void)
{
	...
	IN_channel = KeyBuffValue.InputKey[0];
    mapp_SetLedState(led_name_LedInput1+KeyBuffValue.InputKey[0]-1-10*g_u8Input10PlusCnt,TRUE);
    if(g_u8Input10PlusCnt)
        mapp_SetLedState(led_name_LedInput10Plus,TRUE);
...
void EnterSceneCallStatusHandle(void)
{
    ...
    if(IN_channel<=32&&IN_channel>=1)
    {
        ...
        {
            app_MUH1616S_INPUTSceneCall(IN_channel-1);
...
void SceneIdxOutputStatusHandle(void)
{
    ...
    if(KeyBuffValue.OutputKey[0]>10)
    {
        mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
        mapp_SetLedState(led_name_LedOutput1+KeyBuffValue.OutputKey[0]-1-10,TRUE);
    }
    else
...
void EnterSceneSaveStatusHandle(void)
{
    ...
    if(IN_channel<=32&&IN_channel>=1)
    {
        app_MUH1616S_INPUTSceneSave(IN_channel-1);
...
void Input10PlusStatusHandle(void)
{
    u8 i;
    g_u8Input10PlusCnt = 1;
...
void Output10PlusStatusHandle(void)
{
    g_u8Output10PlusCnt = 1;
    mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
}

void FirstOutput10PlusStatusHandle(void)
{
    g_u8Output10PlusCnt = 1;
    mapp_SetLedState(led_name_LedOutput10Plus,TRUE);
}
...
//    {EnterEdidStatus, EnterEdidStatusHandle},
    ...
    {Input10PlusStatus,Input10PlusStatusHandle},
    {Output10PlusStatus,Output10PlusStatusHandle},
    {FirstOutput10PlusStatus,FirstOutput10PlusStatusHandle},
};
...
void OperateStatsu(uint8_t keyValue)
{
    ...
    else if(keyValue > IN_MAX && keyValue <=OUT_MAX) //11~20 out
    {
        for (i = 0; i < Def_outport_num; i++ )
        {
            if ((i+IN_MAX+1)  == keyValue)
            {
                erroFlag = 1;
                if (KeyStatus == Empt
                    ||KeyStatus == FirstOutput10PlusStatus
                    ||KeyStatus == FirstOutputSta)
                {
                    KeyStatus = FirstOutputSta;
...
//            KeyStatus = EnterSceneSaveStatus;
    ...
    else if(keyValue == IN10PLUS)
    {
        KeyStatus = Input10PlusStatus;
    }
    else if(keyValue == OUT10PLUS)
    {
        if(KeyStatus == Empt
            ||KeyStatus == FirstOutput10PlusStatus
            ||KeyStatus == FirstOutputSta)
...
//    else if(keyValue == CHCEKOUT10PLUS)
//    {
//            KeyStatus = Output10PlusStatus;
//    }
...
void LCD_RecoverToDefault(uint16_t u16Timeout)
{
    ...
    mapp_MUH1616S_INPUT_PlatformTimerSet(emUserTimer_LcdRefreshTime,u16Timeout);
}

void LED_RecoverToDefault(uint16_t u16Timeout)
{
    UITimeOutBuff.LEDStar = 1;
    mapp_MUH1616S_INPUT_PlatformTimerSet(emUserTimer_LedRefreshTime,u16Timeout);
}
...
void led_handle(void)
{
    ...
    {
        if(mapp_MUH1616S_INPUT_PlatformTimerExpired(emUserTimer_LedRefreshTime))
        {
            for(i=led_name_LedInput1;i<led_name_End;i++)
            {
                mapp_SetLedState(i,FALSE);
    ...
    {
        if(mapp_MUH1616S_INPUT_PlatformTimerExpired(emUserTimer_LcdRefreshTime))
        {
            pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
            pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
            ...
            if((pstPrjNameSet->sPrjName[0]!=0&&pstPrjNameSet->sPrjName[1]!=0)&&(pstPrjNameSet->m_u8Len!=0))
            {
                memset(l_cDisplayText,' ',16);
                strncpy(l_cDisplayText, (char*)pstPrjNameSet->sPrjName,pstPrjNameSet->m_u8Len);
            }
            else
            {
                strncpy(l_cDisplayText, LcdNone,16);
