...
#include "../USER/project_file/FIH2RX/mapp_UartCmdList_FIH2RX.h"
...
static emCmdFuncStaType mapp_CmdfuncSwitchInputPort(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    l_bUpdateFlag = TRUE;
    mapp_F1H2RXSourceChange(0+1,l_s8Input);
    pstBasicVar->u8InputSrc = l_s8Input-1;
    if(l_bUpdateFlag)
    {
        mapp_F1H2RXFpgaWinCmdUpdate(10);
    }
    mapp_F1H2RX_PlatformTimerSet(emUserTimer_TimingChange,1000);
    SetUserData_SpecBasicVar();
...
static emCmdFuncStaType mapp_CmdfuncSetTxResolution(pstUartCom_t pCommVar,u8 cmdnum)
{
    ...
    if((l_s8ResID>=1&&l_s8ResID<=emRes_max))
    {
        ...
        mapp_F1H2RXResolutionConfig(0,l_s8ResID-1,TRUE);
