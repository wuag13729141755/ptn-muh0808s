...
stProjectSpecVar_t  stF1H2RXSpecVar;

static volatile u32 g_F1H2RX_Timers[emUserTimer_Max];
...
const uint8_t edid_F1H2RX_default[256] = // default edid
{
...
const stWinTypeFactorType_t cg_stF1H2RXClipWinFactoryType_t[][4]=
{
    //default
    {
...
void mapp_F1H2RX_GsvFunctionInit(void);
void mapp_F1H2RX_GsvFunctionProcess(void);
void mapp_F1H2RXIDT5VInit(void);
void SwitchResolutionToRefreshWindowns(u8 u8Out,u8 u8ResID,bool bUpdate);
void mapp_F1H2RXStartupToRecoveryState(void);
...
//------------------------------------------------------------------------------
void mapp_F1H2RX_PlatformTimerSet( uint8_t index, u32 m_sec )
{
    switch ( index )
    {
        default:
...
//------------------------------------------------------------------------------
u32 mapp_F1H2RX_PlatformTimerGet(uint8_t eTimer)
{
    return(g_F1H2RX_Timers[eTimer]);
}
...
// Description: 定时器初始化
//------------------------------------------------------------------------------
void mapp_F1H2RX_PlatformTimerInit (void)
{
    int i;
...
// Description: 定时器递减函数
//------------------------------------------------------------------------------
void mapp_F1H2RX_PlatformTimerTask(void)
{
    int i;
    for(i=0;i<emUserTimer_Max;i++)
    {
        if(g_F1H2RX_Timers[i] > 0)
        {
            g_F1H2RX_Timers[i]--;
        }
    }
...
//------------------------------------------------------------------------------
uint8_t mapp_F1H2RX_PlatformTimerExpired ( uint8_t timer )           //定时时间到函数
{
    if ( timer < emUserTimer_Max )
    {
        return( g_F1H2RX_Timers[ timer ] == 0);              //定时时间到时返回1
    }

    return( 0 );                                           //定时时间还没到时返回0
}
//========================================================================================
void F1H2RX_1msTimerIrq(void)
{
    mapp_F1H2RX_PlatformTimerTask();
}

void F1H2RX_SpecificVarInit(void)
{
    pstProjectGlobalAllVar_t    pstPrjAllVar = &g_stPrjGloVar;
    pstProjectGlobalSaveVar_t   pstPrjSaveVar = &pstPrjAllVar->stPrjSaveVar;
    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
...
void F1H2RX_SpecificVarDefault(void)
{
    F1H2RX_SpecBasicVarDefault();
    F1H2RX_SpecCustomEdidVarDefault();
}

void F1H2RX_SpecBasicVarDefault(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
    ...
    pstSpecVar->stBasSpecVar.u8TxResolutionID[0] = emRes_1920x1080_60;
    pstSpecVar->stBasSpecVar.u8TxHdcpMode[0] = 0;
    pstSpecVar->stBasSpecVar.u8InputSrc = 0;
}

void F1H2RX_SpecCustomEdidVarDefault(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
...
bool F1H2RX_SpecVarValidCheck(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
...
void F1H2RX_PrjStatePowerOnInit(void)
{
    System_Default_Edid_table = (iTE_u8 *)edid_F1H2RX_default;
    memcpy(g_pu8It663xxEdidBuffer, System_Default_Edid_table,256);

    // prj 1ms loop hook
    pFuncPrj1MsLoopHook = F1H2RX_1msTimerIrq;
    pvvFuncGsvUserFunctionInit = mapp_F1H2RX_GsvFunctionInit;
    pvvFuncGsvUserFunctionProcess = mapp_F1H2RX_GsvFunctionProcess;
//    pvvInitIDTFunc = mapp_F1H2RXIDT5VInit;
    mapp_RevisionMake();

    mapp_SetTx5VControl(TRUE);
}

void F1H2RX_LedWork_Loop(void)
{
    static bool sl_bLedSta = FALSE;
    if(!mapp_F1H2RX_PlatformTimerExpired(emUserTimer_LedActive))
        return;
    mapp_F1H2RX_PlatformTimerSet(emUserTimer_LedActive,500);
...
void mapp_F1H2RX_Gsv2K11FunctionInit(void)
{
    uint8 i=0;
#if GSV2K11
    ...
    for(i=0;i<AvGsv2k11DeviceNumber;i++)
    {
        ...
        gsv2k11Ports[i][6].content.vg->Pattern       = AV_PT_COLOR_BAR;//AV_PT_RED_RAMP;//AV_PT_COLOR_BAR;//
    #endif
...
void mapp_F1H2RX_GsvFunctionInit(void)
{
    mapp_F1H2RX_Gsv2K11FunctionInit();
...
#else   // 切换的时候会有识别到黑场，这时候发送开窗命令会出现绿屏现象，这里增加检测1秒内为黑场，则为真的无信号，否则为信号切换状态
    {
        static u8 sl_u8CheckCnt = 0;

        if((g_stGsvRxTiming.u16VActive != g_stRxResTable[0].u16Height)
            ||(g_stGsvRxTiming.u16HActive != g_stRxResTable[0].u16Width)
            ||(g_stGsvRxTiming.u8VClk != g_stRxResTable[0].u16VFreq))
        {
            if(++sl_u8CheckCnt>=100)
            {
                sl_u8CheckCnt = 0;
                g_stRxResTable[0].bChangeFlag = TRUE;
...
void F1H2RX_FuncWork_LoopRunning(void)
{
    F1H2RX_LedWork_Loop();

    mapp_F1H2RXStartupToRecoveryState();
    mapp_SourceChangeToSetWinProcess();
    report_video_timing();
}

void mapp_F1H2RXStartupMessage(void)
{
    RS232_printf("\r\n");
...
// FPGA config
//================================================================================
void mapp_F1H2RX_SendDataToFpag(pu8 pu8Data, u16 u16Len)
{
...
void send_cmd_to_fpga(uint8_t u8Addr,uint8_t u8Cmd,uint8_t* pu8Data,uint16_t u16Len)
{
    ...
    mapp_F1H2RX_SendDataToFpag(l_u8Buff,7+u16Len);
}

void mapp_F1H2RX_SetAudioSource(u8 u8Out, u8 u8AudioSrc)
{
    u8 l_u8Temp[2];
...
void mapp_F1H2RXWinDataCopy(u8 *u8Data, pstWindowsConfig_t pstWincfgDat)
{
...
u16 mapp_F1H2RXGetClipWinStartX(u8 u8Type,u8 u8Output,u16 u16Width)
{
    u16 l_u16Temp = 0;

    if(cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StX_sf2 == 0)
    {
        l_u16Temp = 0;
    }
    else
    {
        l_u16Temp = (cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StX_sf1*u16Width)
                    /cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StX_sf2;
    }
...
u16 mapp_F1H2RXGetClipWinStartY(u8 u8Type,u8 u8Output,u16 u16Height)
{
    u16 l_u16Temp = 0;

    if(cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StY_sf2 == 0)
    {
        l_u16Temp = 0;
    }
    else
    {
        l_u16Temp = (cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StY_sf1*u16Height)
                    /cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8StY_sf2;
    }
...
u16 mapp_F1H2RXGetClipWinWidth(u8 u8Type,u8 u8Output,u16 u16Width)
{
    u16 l_u16Temp = u16Width;

    if(cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Width_sf2 == 0)
    {
        l_u16Temp = u16Width;
    }
    else
    {
        l_u16Temp = (cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Width_sf1*u16Width)
                    /cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Width_sf2;
    }
...
u16 mapp_F1H2RXGetClipWinHeight(u8 u8Type,u8 u8Output,u16 u16Height)
{
    u16 l_u16Temp = u16Height;

    if(cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Height_sf2 == 0)
    {
        l_u16Temp = u16Height;
    }
    else
    {
        l_u16Temp = (cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Height_sf1*u16Height)
                    /cg_stF1H2RXClipWinFactoryType_t[u8Type][u8Output].u8Height_sf2;
    }

    return l_u16Temp;
}

void mapp_F1H2RXWindowCfg(uint8_t out_ch,uint8_t in_ch)
{
//    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
...
//    stWinConfig.u16Src_V    = (g_stWinConfigVar[out_ch-1].stClipWin.u16Height)?(g_stWinConfigVar[out_ch-1].stClipWin.u16Height):(stWinConfig.u16SrcAct_V);
    stWinConfig.u16Src_X    = mapp_F1H2RXGetClipWinStartX(l_u8Mode,out_ch-1,stWinConfig.u16SrcAct_H);
    stWinConfig.u16Src_Y    = mapp_F1H2RXGetClipWinStartY(l_u8Mode,out_ch-1,stWinConfig.u16SrcAct_V);
    stWinConfig.u16Src_H    = mapp_F1H2RXGetClipWinWidth(l_u8Mode,out_ch-1,stWinConfig.u16SrcAct_H);
    stWinConfig.u16Src_V    = mapp_F1H2RXGetClipWinHeight(l_u8Mode,out_ch-1,stWinConfig.u16SrcAct_V);
...
//    dbg_printf("data size  == %d \r\n",sizeof(stWindowsConfig_t));
    ...
    mapp_F1H2RXWinDataCopy(l_u8Temp,&stWinConfig);
    send_cmd_to_fpga(out_ch,eFpgaRegOutWinCfg,l_u8Temp,43);

    return;
}

void mapp_F1H2RXFpgaWinCmdUpdate(u32 u32DelayMs)
{
    u8 l_u8cmd = 1;
...
void mapp_F1H2RX_TimingConfig(uint8_t ch,uint8_t idx)
{
    u8 l_u8Temp[50];
...
void mapp_F1H2RXResolutionConfig(u8 u8Out,u8 u8ResID,bool bUpdate)
{
    const u8 cl_u8ChMap[1] = {3};
    mapp_F1H2RX_TimingConfig(u8Out+1,u8ResID);
...
void SwitchResolutionToRefreshWindowns(u8 u8Out,u8 u8ResID,bool bUpdate)
{
    ...
    mapp_F1H2RXWindowCfg(1,pstBasicSpecVar->u8InputSrc+1);
    pstBasicSpecVar->u8TxResolutionID[0]=u8ResID;

    if(bUpdate)
    {
        mapp_F1H2RXFpgaWinCmdUpdate(10);
    }
}

void mapp_F1H2RXSourceChange(u8 u8Out,u8 u8In)
{
    mapp_F1H2RXWindowCfg(u8Out,u8In);
    mapp_F1H2RX_SetAudioSource(u8Out,u8In);
}
...
void mapp_F1H2RXStartupToRecoveryState(void)
{
    ...
    if(!mapp_F1H2RX_PlatformTimerExpired(emUserTimer_StartupTime))
        return;
        ...
        case eSt_GetTxVersion:
            {
                ...
                delay_ms(50);
                mapp_F1H2RX_PlatformTimerSet(emUserTimer_StartupTime,200);
                sl_u8LoopCnt++;
        ...
        case eSt_SetTxResolution:
            {
                ...
                {
                    mapp_F1H2RXResolutionConfig(i,pstBasicVar->u8TxResolutionID[i],(i==(Def_outport_num-1))?TRUE:FALSE);
                }
                delay_ms(50);
                mapp_F1H2RX_PlatformTimerSet(emUserTimer_StartupTime,500);
                mapp_SetFreezeTime(100);
                mapp_SetFreeze(0);
                sl_eStartupLoop++;
            }
            break;
...
void mapp_SourceChangeToSetWinProcess(void)
{
    ...
    if(!mapp_F1H2RX_PlatformTimerExpired(emUserTimer_TimingChange))
        return;
    mapp_F1H2RX_PlatformTimerSet(emUserTimer_TimingChange,SRC_CHG_LOOP_CNT);

    for(l_u8Src = 0;l_u8Src<2;l_u8Src++)
    {
        if(l_u8Src==1)
        {
            if(mapp_it66021_GetVideoChangeSta(0) != emIt66021_VidChgNone)
                sl_u8SrcChgCnt[l_u8Src]++;
            if(sl_u8SrcChgCnt[l_u8Src]>=DET_INPUT_CNT)
            //if(mapp_it66021_GetVideoChangeSta(0) != emIt66021_VidChgNone)
            {
                sl_u8SrcChgCnt[l_u8Src] = 0;
                it66021_GetInputVFreq(0,&l_u8Vfreq);
                g_stRxResTable[l_u8Src].u8ScanMode = it66021_GetInputTiming(0,&l_u16HActive,&l_u16VActive,&l_u32Pclk);
                ...
                for(l_u8OutIdex = 0; l_u8OutIdex<Def_outport_num;l_u8OutIdex++)
                {
                    if(pstBasicSpecVar->u8InputSrc == (l_u8Src))
                    {
                        ...
                        l_bUpdateFlag = TRUE;
                        mapp_F1H2RXWindowCfg(l_u8OutIdex+1,(l_u8Src+1));
                        mapp_F1H2RX_SetAudioSource(l_u8OutIdex+1,(l_u8Src+1));
//                        g_u8BlackScreenSta = 0;
                ...
                mapp_it66021_SetVideoChangeSta(0, (u8)emIt66021_VidChgNone);
                g_bReportTimingFlag[l_u8Src] = TRUE;
                mapp_F1H2RX_PlatformTimerSet(emUserTimer_ReportTiming,500);
            }
        }
        else
        {
...
//                g_bReportTimingFlag[l_u8Src] = TRUE;
//                mapp_F1H2RX_PlatformTimerSet(emUserTimer_ReportTiming,0);
//                report_video_timing();
                if(l_bUpdateFlag)
                {
                    mapp_F1H2RXFpgaWinCmdUpdate(10);
        ...
        if((g_stRxResTable[l_u8Src].u16Width==0)&&(g_stRxResTable[l_u8Src].u16Height==0))
        {
            ...
            if(sl_u16SrcLossCnt[l_u8Src] >= (10000/SRC_CHG_LOOP_CNT))
            {
                if(pstBasicSpecVar->u8InputSrc == (l_u8Src))
                {
...
#define REPORT_TIMING_TIME  3000//1000
void report_video_timing(void)
{
    pstProjectSpecVar_t         pstSpecVar = &stF1H2RXSpecVar;
    pstBasicSpecVar_t           pstBasicSpecVar = &pstSpecVar->stBasSpecVar;
    ...
    if(!mapp_F1H2RX_PlatformTimerExpired(emUserTimer_ReportTiming))
        return;
    mapp_F1H2RX_PlatformTimerSet(emUserTimer_ReportTiming,REPORT_TIMING_TIME);
    ...
    for(i=0;i<_HDMI_INPUT_PORT_USED_NUMBER;i++)
    {
        if(i==1)
        {
            ...
            it66021_GetInputVFreq(0,&l_u8Vfreq);
            g_stRxResTable[i].u8ScanMode = it66021_GetInputTiming(0,&l_u16HActive,&l_u16VActive,&l_u32Pclk);
