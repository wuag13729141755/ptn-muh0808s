...
/* for reference , define in iTE6615_DEV_DEFINE.h
typedef struct {
    u16 HActive ;
    u16 VActive ;
    u16 HTotal ;
...
// this table for sync embedded need to re-gen sync detail by input VIC (sync-embedded only know Data-Enable, need to set sync by sw)
...
u32 iTE6615_Get_VIC_Pointer(u16 vic);
u16  iTE6615_Get_VIndex_ByTiming(_CODE iTE6615_VTiming *ct);
u8  _value_in_range( u32 value, u32 comp, u32 tor_percent );



u8 iTE6615_Get_TimingByVIC(u32 vic)
{
    u32 i = 0;
    if(vic==0) return FALSE;

    i = iTE6615_Get_VIC_Pointer(vic);
    if(i == VTableCount)
    {
        return FALSE;
    }
    iTE6615_InputTiming = &iTE6615_VTable[i];

    if ( iTE6615_InputTiming )
    {
        HDMITX_DEBUG_PRINTF(("copy vt, vic=%d\n", iTE6615_InputTiming->VIC));
        return TRUE;
    }
    else
    {
        i = iTE6615_Get_VIC_Pointer(4);
        iTE6615_InputTiming = &iTE6615_VTable[i];
        HDMITX_DEBUG_PRINTF(("copy vt, vic=%d\n", iTE6615_InputTiming->VIC));
        return FALSE;
    }
