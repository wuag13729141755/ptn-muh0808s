...
extern _CODE iTE6615_VTiming  *iTE6615_InputTiming;

#if ENABLE_HDCP22_SWITCH
static u8 sg_u8HdcpType = 0;
#endif
...
u8 iTE6615_Check_RCLK_Valid( u32 rclk )
{
    ...
    HDMITX_AUDIO_PRINTF(("RCLK=%ld, max=%ld, min=%ld, avg=%ld, range=%ld\r\n", rclk, (u32)RCLK_MAX / 1000, (u32)RCLK_MIN / 1000, (u32)RCLK_TYPICAL / 1000, (u32)RCLK_DIFF));
    return FALSE;
}
...
u8 iTE6615_CheckConfig_Video(void)
{
    ...
    if (iTE6615_DATA.config.Input_ColorSpace != ITE6615_YUV420 &&
        iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV420)
    {
    ...
    {
        if (iTE6615_DATA.config.Output_ColorSpace == ITE6615_RGB444 &&
            iTE6615_DATA.config.Output_ExtendedColorimetry_Format != ITE6615_AdobeRGB &&
            iTE6615_DATA.config.Output_ExtendedColorimetry_Format != ITE6615_BT2020YCC) {
            HDMITX_DEBUG_PRINTF(("ERROR: ITE6615_RGB444 must set Output_ExtendedColorimetry_Format to ITE6615_AdobeRGB or ITE6615_BT2020YCC !!!\r\n"));
            flag = FALSE;
        }

        if (iTE6615_DATA.config.Output_ColorSpace != ITE6615_RGB444 &&
            iTE6615_DATA.config.Output_ExtendedColorimetry_Format == ITE6615_AdobeRGB) {
            HDMITX_DEBUG_PRINTF(("ERROR: YCbCr can not set Output_ExtendedColorimetry_Format to ITE6615_AdobeRGB !!!\r\n"));
            flag = FALSE;
        }
    }
    ...
    {
        iTE6615_DATA.config.Input_ColorSpace = ITE6615_YUV422;
    }
    ...
    (iTE6615_DATA.config.Input_ColorSpace == ITE6615_YUV420 &&iTE6615_DATA.config.Output_ColorSpace != ITE6615_YUV420))
    {
        iTE6615_DATA.config.EnH2InLBBypass = FALSE;
    }
...
void iTE6615_CheckConfigVIC(void)
{
    #if _ENABLE_AUTO_SET_VIC
    // Auto Set VIC
    ...
    #endif
	iTE6615_DATA.Flag_Force_Set_VIC_0 = FALSE;
...
u8 iTE6615_CheckID(void)
{
	u8 temp = ((iTE6615_DATA.ChipID&0xF0)>>4)&0x0F;
...
void iTE6615_SetTX_Power(u8 POWER_STATE)
{
    if(POWER_STATE == POWER_ON)
    {
        ...
        //lvrx pll on
	    lvrxset(0x70, 0x01, 0x01);	// L01_XP_RSTB
	    lvrxset(0x72, 0x01, 0x01);	// L23_XP_RSTB
        ...
        hdmitxset(0x26, 0x31, 0x00);   // PwrOn GRCLK
        hdmitxset(0x88, 0x08, 0x00);   // PwrDn DRV
        hdmitxset(0x8e, 0x60, 0x60);   // PwrDn DRV
    ...
    else
    {
		...
		// new
        hdmitxset(0x8e, 0x60, 0x00);   // PwrDn DRV
        hdmitxset(0x88, 0x08, 0x08);   // PwrDn DRV
		hdmitxset(0x25, 0x0E, 0x0E);   // PwrDn ICLK, IACLK, TXCLK
		hdmitxset(0x26, 0x31, 0x31);   // PwrDn FPCLK, FTXCLK
		hdmitxset(0xFC, 0x01, 0x00);   // PwrDn CRCLK
		//hdmitxset(0x25, 0x01, 0x01);   // PwrDn GRCLK
        // mark by kuro due to 6264 couldn't light up because of this
    }
}
...
void iTE6615_SetTX_AFE( void )
{
    ...
    if( iTE6615_DATA.config.Input_ColorSpace==ITE6615_YUV420 && iTE6615_DATA.config.Output_ColorSpace!=ITE6615_YUV420 )
    ...
    else if( iTE6615_DATA.VCLK_expected > ((u32)150000) ) // single-end swing = 450mV
    {
        ...
        DRV_TPRE    = 0;
        DRV_NOPE    = 1;
        H2ON_PLL    = 0;         // TPRE4 = 1 for CK channel
        #endif
    }
    else // single-end swing = 500mV
    {
        ...
        DRV_TPRE    = 0;
        DRV_NOPE    = 1;
        H2ON_PLL    = 0;         // TPRE4 = 0 for CK channel
        #endif
    }
...
void iTE6615_SetTX_Reset(void)
{
    ...
    hdmitxwr(0xFF, 0xC3);
    hdmitxwr(0xFF, 0xA5);
    hdmitxset(0x0C, 0x1C, (config_ForceRxOn << 2)|(config_ForceTMDSStb<<4)|(config_ForceVOut<<3));
    hdmitxwr(0xF8, 0xFF);
    ...
    if(iTE6615_CheckID())
	{
		hdmitxwr(0x24, (_0x10_TTL_EnICLKHInv<<7)+(_0x10_TTL_ICLKHDly<<4)+(_0x10_TTL_EnICLKLInv<<3)+_0x10_TTL_ICLKLDly);
	}
	else
	{
    ...
    hdmitxset(0x35, 0x03, iTE6615_DATA.config.u8DDCSpeed);
    //hdmitxset(0x3A, 0xFC, 0xD0);
    hdmitxset(0x3A, 0xFC, 0x90|(config_DisH2Auto<<6));

    // Input TTL setting
    hdmitxset(0x90, 0x7F, (_TTL_Default_RegCtrlHSel_<<6)+(_TTL_Default_RegCtrlLSel_<<5)+(1<<4)+(config_InSyncAutoSwapOff<<3)+(iTE6615_DATA.config.Input_Enable_DualMode_Mode<<2)+(iTE6615_DATA.config.Input_Enable_HL_Swap<<1)+iTE6615_DATA.config.Input_Enable_DualMode);
    ...
    hdmitxset(0xA0, 0x20, (_TTL_Enable_InputYUV422DataRounding_<<5));

	if(iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV422)
	{
		hdmitxset(0xA1, 0xF0, 0xC0+(iTE6615_DATA.config.TTL_Default_YUV422ReceiveBits<<4));
	}
	else
	{
		hdmitxset(0xA1, 0xF0, 0x40+(iTE6615_DATA.config.Output_ColorDepth<<4));
	}
    ...
    if(iTE6615_DATA.config.Enable_VFrontProchChange == 1)
	{
		hdmitxset(0x95, 0x80, 0x80);
		hdmitxset(0x0C, 0x08, 0x08);
	}
	else
	{
		hdmitxset(0x95, 0x80, (iTE6615_DATA.config.EnH2InLBBypass << 7));
		hdmitxset(0x0C, 0x08, 0x00);
	}
    ...
    hdmitxset(0x19, 0xff, 0x03);   // Enable HPD and RxSen Interrupt
    hdmitxset(0xc0, 0x28, 0x20);
    hdmitxset(0xcf, 0x3F, 0x08);

	// CTS may need to change from 0x34 to 0x33 for deglitch or HPD low 100ms couldn't trigger any Int
	hdmitxset(0xf4, 0x77, 0x32);
...
void iTE6615_SetTX_EmbeddedFormat(u8 auto_get_vic)
{
    // using for sync-embedded/pattern gen
    u32 HTotal, HFP, HSW, HBP, HDEW, HVR2nd;
    u32 VTotal, VFP, VSW, VBP, VDEW, VFP2nd;
    u32 HPol, VPol, Prog;
    u8 pixel_repeat = iTE6615_DATA.config.PixRpt;

    if(iTE6615_DATA.config.TTL_Enable_SyncEmbedd == 0)return;

    if( !auto_get_vic || iTE6615_Get_TimingByVIC(iTE6615_CurVTiming.VIC) )
    {
        HTotal = iTE6615_InputTiming->HTotal / pixel_repeat;
        HDEW   = iTE6615_InputTiming->HActive / pixel_repeat;

        HFP    = iTE6615_InputTiming->HFrontPorch / pixel_repeat;
        HSW    = iTE6615_InputTiming->HSyncWidth / pixel_repeat;
        HBP    = iTE6615_InputTiming->HBackPorch / pixel_repeat;

        VFP    = iTE6615_InputTiming->VFrontPorch;
        VSW    = iTE6615_InputTiming->VSyncWidth;
        VBP    = iTE6615_InputTiming->VBackPorch;
        VDEW   = iTE6615_InputTiming->VActive;
        HPol   = iTE6615_InputTiming->HPolarity;
        VPol   = iTE6615_InputTiming->VPolarity;
        Prog   = iTE6615_InputTiming->ScanMode;

        // H2TX PatGen Parameter
        if( !iTE6615_InputTiming->ScanMode)
        {
            if( iTE6615_CurVTiming.VIC==39 ) {
                HVR2nd = HTotal/2;
                VFP2nd = iTE6615_InputTiming->VFrontPorch-1;
                VTotal = iTE6615_InputTiming->VTotal*2;
            }
            else {
                HVR2nd = HTotal/2;
                VFP2nd = iTE6615_InputTiming->VFrontPorch;
                VTotal = iTE6615_InputTiming->VTotal*2+1;
            }
        }
        else
        {
            HVR2nd = 0xFFF;
            VFP2nd = 0xFFF;
            VTotal = iTE6615_InputTiming->VTotal;
        }

        if(iTE6615_DATA.config.Infoframe_Enable3D)
        {
            if(iTE6615_DATA.config.Infoframe_3D_Structure == _3D_SideBySide_Full)
            {
                HTotal = HTotal*2;
                HFP = HFP*2;
                HSW = HSW*2;
            ...
            if(iTE6615_DATA.config.Infoframe_3D_Structure == _3D_FramePacking)
            {
                if( Prog )
                {
                    VTotal = VTotal*2;
                ...
                {
                    VTotal = iTE6615_InputTiming->VTotal*4+2;
                    VDEW = iTE6615_InputTiming->VActive*4+(iTE6615_InputTiming->VBackPorch+iTE6615_InputTiming->VFrontPorch+iTE6615_InputTiming->VSyncWidth)*3+2;
                }
            }
        }
        ...
        chgtxbank(2);
        // HFP
        hdmitxwr(0x10, HFP&0xFF);
        //h2txwr(0x10, 0x3A);
        hdmitxwr(0x11, (HFP&0x3F00)>>8);
        // HSW
        hdmitxwr(0x12, HSW&0xFF);
        //h2txwr(0x12, 0x14);
        hdmitxwr(0x13, (HSW&0x3F00)>>8);
        // HBP
        hdmitxwr(0x14, HBP&0xFF);
        hdmitxwr(0x15, (HBP&0x3F00)>>8);
        // HDEW
        hdmitxwr(0x16, HDEW&0xFF);
        hdmitxwr(0x17, (HDEW&0x3F00)>>8);
        // HVR2nd
        hdmitxwr(0x18, HVR2nd&0xFF);
        hdmitxwr(0x19, (HVR2nd&0x3F00)>>8);
        // VFP
        hdmitxwr(0x1A, VFP&0xFF);
        //h2txwr(0x1A, 0x04);
        hdmitxwr(0x1B, (VFP&0x1F00)>>8);
        // VSW
        hdmitxwr(0x1C, VSW&0xFF);
        //h2txwr(0x1C, 0x06);
        hdmitxwr(0x1D, (VSW&0x1F00)>>8);
        // VBP
        hdmitxwr(0x1E, VBP&0xFF);
        hdmitxwr(0x1F, (VBP&0x1F00)>>8);
        // VDEW
        hdmitxwr(0x20, VDEW&0xFF);
        hdmitxwr(0x21, (VDEW&0x1F00)>>8);
        // VFP2nd
        hdmitxwr(0x22, VFP2nd&0xFF);
        //h2txwr(0x22, 0x04);
        hdmitxwr(0x23, (VFP2nd&0x1F00)>>8);
        // Sync Polarity
        hdmitxset(0x2E, 0x07, ((!Prog)<<2)+(VPol<<1)+HPol);

        hdmitxset(0x30, 0x40, 0x40);
        hdmitxwr(0x31, 0x00);
        hdmitxwr(0x32, 0x80);
        hdmitxwr(0x33, 0x80);
        hdmitxwr(0x34, 0x80);
...
void iTE6615_Get_TXCLK()
{
    ...
    It6615_delay1ms(1); // add for counter done or TXCLK getting error
...
void    iTE6615_Get_VideoInfo(void)
{
    ...
    if(iTE6615_DATA.config.Input_ColorSpace == ITE6615_YUV420 && iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV420) need_double = 1;
    else need_double = 0;
...
void iTE6615_Show_VideoInfo(void)
{
    ...
    {
        case ITE6615_RGB444 :
            HDMITX_DEBUG_PRINTF(("\tCSC = ITE6615_RGB444\r\n")); break;
        case ITE6615_YUV422 :
            HDMITX_DEBUG_PRINTF(("\tCSC = ITE6615_YUV422\r\n")); break;
        case ITE6615_YUV444 :
            HDMITX_DEBUG_PRINTF(("\tCSC = ITE6615_YUV444\r\n")); break;
        default :
            HDMITX_DEBUG_PRINTF(("\tCSC = ITE6615_YUV420\r\n")); break;
    }

    if (iTE6615_DATA.config.Input_ColorSpace)
    {
        if (iTE6615_DATA.config.Input_YCC_Colorimetry==ITE6615_ITU709)
            HDMITX_DEBUG_PRINTF(("\tYUVColorimetry = ITE6615_ITU709\r\n"));
        else
            HDMITX_DEBUG_PRINTF(("\tYUVColorimetry = ITE6615_ITU601\r\n"));
    }
    ...
    if( iTE6615_DATA.config.Output_ExtendedColorimetry_Enable )
    {
        ...
        {
            case ITE6615_xvYCC601    : HDMITX_DEBUG_PRINTF(("ITE6615_xvYCC601")); break;
            case ITE6615_xvYCC709    : HDMITX_DEBUG_PRINTF(("ITE6615_xvYCC709")); break;
            case ITE6615_sYCC601     : HDMITX_DEBUG_PRINTF(("ITE6615_sYCC601")); break;
            case ITE6615_AdobeYCC601 : HDMITX_DEBUG_PRINTF(("ITE6615_AdobeYCC601")); break;
            case ITE6615_AdobeRGB    : HDMITX_DEBUG_PRINTF(("ITE6615_AdobeRGB")); break;
            case ITE6615_BT2020cYCC  : HDMITX_DEBUG_PRINTF(("ITE6615_BT2020cYCC")); break;
            case ITE6615_BT2020YCC   : HDMITX_DEBUG_PRINTF(("ITE6615_BT2020YCC")); break;
            default          : break;
        }
        HDMITX_DEBUG_PRINTF(("\r\n"));
    }

    HDMITX_DEBUG_PRINTF(("\tPCLK = %u.%ld MHz\r\n\r\n", iTE6615_DATA.VCLK/1000,iTE6615_DATA.VCLK%1000));
	...
	HDMITX_DEBUG_PRINTF(("receive mode = %d (0=oddeven, 1=leftright)\r\n", (int) iTE6615_DATA.config.Input_Enable_DualMode_Mode));
    HDMITX_DEBUG_PRINTF(("HDMI repeat times = %d\r\n", (int) iTE6615_DATA.config.PixRpt));
    ...
    HDMITX_DEBUG_PRINTF(("\r\n"));
	HDMITX_DEBUG_PRINTF(("LVDS Map : %d (0 : JEIDA, 1 : VESA)",(int) iTE6615_DATA.config.LVDS_RxOutMap));
    HDMITX_DEBUG_PRINTF(("\r\nSSCPCLKL=%ld MHz, SSCPCLKH=%ld MHz\r\n", iTE6615_DATA.SSCPCLKL/1000, iTE6615_DATA.SSCPCLKH/1000));

    #endif

    HDMITX_DEBUG_PRINTF(("\r\n"));
    HDMITX_DEBUG_PRINTF(("RCLK=%ld.%ld MHz\r\n", iTE6615_DATA.RCLK / 1000,iTE6615_DATA.RCLK % 1000));
    HDMITX_DEBUG_PRINTF(("ICLK=%ld.%ld MHz\r\n", iTE6615_DATA.ICLK / 1000,iTE6615_DATA.ICLK % 1000));
    HDMITX_DEBUG_PRINTF(("PCLK=%ld.%ld MHz\r\n", iTE6615_DATA.PCLK / 1000,iTE6615_DATA.PCLK % 1000));
    HDMITX_DEBUG_PRINTF(("TXCLK=%ld.%ld MHz\r\n",iTE6615_DATA.TXCLK / 1000,iTE6615_DATA.TXCLK % 1000));
    HDMITX_DEBUG_PRINTF(("VCLK=%ld.%ld MHz\r\n", iTE6615_DATA.VCLK / 1000,iTE6615_DATA.VCLK % 1000));
    HDMITX_DEBUG_PRINTF(("VCLK_expected=%ld.%ld MHz\r\n", iTE6615_DATA.VCLK_expected / 1000,iTE6615_DATA.VCLK_expected % 1000));
    HDMITX_DEBUG_PRINTF(("\r\n"));
...
{
    u8 RxHDCP2;
    u8 l_u8EnRepWaitTxMgm;
    u8 l_u8EnRepWaitTxEks;
    u8 HDCP_Enable2ListSRMChk;

    if( (hdmitxrd(0x07)&0x06)!=0x06 ) {
        iTE6615_sys_chg(iTE6615_STATES_Unplug);
        #if _HDCP_Enable_iTE6615_AS_HDCPRepeater_Code_
		if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
		{
			iTE6615_DATA.CB_HDCP.iTE6615_Inform_HPD_Unplug_CB();
		}
		#endif
        return;
    }
    ...
    l_u8EnRepWaitTxMgm = 0;
    l_u8EnRepWaitTxEks = 0;
    ...
    hdmitxset(0x6B, 0x30, (l_u8EnRepWaitTxMgm<<5)+(l_u8EnRepWaitTxEks<<4));
    hdmitxwr(0xFF, 0xFF);
...
#if (!ENABLE_OUTPUT_HDCP22_FOR_NON_4K)
    if ( RxHDCP2 )
    {
        if (iTE6615_CurVTiming.HActive < 3800)
        {
            HDMITX_DEBUG_PRINTF(("Disable HDCP2.2 for non-4k mode HActive=%u\r\n", iTE6615_CurVTiming.HActive));
            RxHDCP2 = 0;
        }
    }
#endif
...
#if _HDCP_Enable_iTE6615_AS_HDCPRepeater_Code_
    // HDCP version assign by repeater
    if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
    {
        if(iTE6615_DATA.HDCP_Enable_HDCPVersion == 1)
        {
            RxHDCP2 = 0;
        }
        else if(iTE6615_DATA.HDCP_Enable_HDCPVersion == 2)
        {
            RxHDCP2 = 1;
        }
        else
...
void iTE6615_ddc_fifo_clear( void )
{
    u8 reg = hdmitxrd(0x28);
    hdmitxwr(0x28, reg|0x01);
    hdmitxwr(0x2E, 0x09);
...
// add by wf8421 20190301
void iTE6615_HDCP_Disable( void )
{
    hdmitxset(0x42, 0x10, 0x00);    // RegHDCPSel=0
    hdmitxset(0x19, 0x04, 0x04);    // Enable DDC Command Fail Interrupt
    hdmitxset(0x1D, 0x08, 0x08);    // Enable DDC Bus Hang Interrupt
    ...
    iTE6615_DATA.vars.RxHDMIMode = 0;
    #endif
    iTE6615_ddc_fifo_clear();
}
#endif

#if ENABLE_HDCP22_SWITCH
void iTE6615_HdcpTypeSwitch(u8 u8HdcpType)
{
    sg_u8HdcpType = u8HdcpType;
}

u8 iTE6615_GetHdcpType(void)
{
    return sg_u8HdcpType;
}
#endif

#if ENABLE_OUTPUT_HDCP_SWITCH
void iTE6615_OutputHdcpSwitch(u8 u8HdcpSta)
{
    if(u8HdcpSta == 0)      // HDCP Disable
    {
        IT6615_MSG(("!!!!!!! HDCP DISABLE\r\n"));
        iTE6615_DATA.config.HDCP_Enable = FALSE;
        iTE6615_hdcp_chg( STATEH_Disable );
    }
    else if(u8HdcpSta == 2)     //HDCP2.2
    {
        IT6615_MSG(("!!!!!!! HDCP2.2 ENABLE\r\n"));
        iTE6615_DATA.config.HDCP_Enable = TRUE;
        iTE6615_HdcpTypeSwitch(1);
        iTE6615_hdcp_chg( STATEH_CPGoing );
    }
    else        // HDCP1.4
    {
        IT6615_MSG(("!!!!!!! HDCP1.4 ENABLE\r\n"));
        iTE6615_DATA.config.HDCP_Enable = TRUE;
        iTE6615_HdcpTypeSwitch(0);
        iTE6615_hdcp_chg( STATEH_CPGoing );
    }
}
#endif
...
u8 iTE6615_HDCPRepeater_CheckSupport( void )
{
    ...
    iTE6615_DATA.Info_HDCPBStatus[0] = hdmitxrd(0x64);
    iTE6615_DATA.Info_HDCPBStatus[1] = hdmitxrd(0x65);

	HDCP_DEBUG_PRINTF(("iTE6615_DATA.Info_HDCPBStatus[0] = 0x%02X \r\n", (int)iTE6615_DATA.Info_HDCPBStatus[0]));
	HDCP_DEBUG_PRINTF(("iTE6615_DATA.Info_HDCPBStatus[1] = 0x%02X \r\n", (int)iTE6615_DATA.Info_HDCPBStatus[1]));


	depth = iTE6615_DATA.Info_HDCPBStatus[1]&0x07;
    HDCP_DEBUG_PRINTF(("iTE: Device Count = %X\r\n", iTE6615_DATA.Info_HDCPBStatus[0]&0x7F));
    HDCP_DEBUG_PRINTF(("iTE: Max. Device Exceeded = %02X\r\n", (iTE6615_DATA.Info_HDCPBStatus[0]&0x80)>>7));


    HDCP_DEBUG_PRINTF(("iTE: Depth = %X\r\n", depth));
    HDCP_DEBUG_PRINTF(("iTE: Max. Cascade Exceeded = %02X\r\n", (iTE6615_DATA.Info_HDCPBStatus[1]&0x08)>>3));
    HDCP_DEBUG_PRINTF(("iTE: HDMI_MODE = %d\r\n", (iTE6615_DATA.Info_HDCPBStatus[1]&0x10)>>4));

    if( (iTE6615_DATA.Info_HDCPBStatus[0]&0x80) || (iTE6615_DATA.Info_HDCPBStatus[1]&0x08) )
	{
        HDCP_DEBUG_PRINTF(("iTE: ERROR: Max. Device or Cascade Exceeded !!!\r\n"));
    }

	// read ksv list from ddc fifo
	dev_count = (iTE6615_DATA.Info_HDCPBStatus[0]&0x7F);

	if( dev_count )
	{
		iTE6615_HDCP_Read(0x43, dev_count*5);     // KSV LIST FIFO
		hdmitxbrd(0x30, dev_count*5, &iTE6615_DATA.KSV_List[0]);
		HDCP_DEBUG_PRINTF(("KSV_List addr = 0x%X \r\n", &iTE6615_DATA.KSV_List[0]));
	}
	else
	{
	...
	for(i=0; i<(iTE6615_DATA.Info_HDCPBStatus[0]&0x7F); i++) {
		m[msgcnt++] = (unsigned char) iTE6615_DATA.KSV_List[i*5+0];
		m[msgcnt++] = (unsigned char) iTE6615_DATA.KSV_List[i*5+1];
		m[msgcnt++] = (unsigned char) iTE6615_DATA.KSV_List[i*5+2];
		m[msgcnt++] = (unsigned char) iTE6615_DATA.KSV_List[i*5+3];
		m[msgcnt++] = (unsigned char) iTE6615_DATA.KSV_List[i*5+4];
	...
	m[msgcnt++] = iTE6615_DATA.Info_HDCPBStatus[0];
	m[msgcnt++] = iTE6615_DATA.Info_HDCPBStatus[1];
...
void iTE6264_Get_SSCPCLK( void )
{
    ...
    HDMITX_DEBUG_PRINTF(("predivsel=%d\r\n", (int)predivsel));
