...
_CODE iTE6615_VTiming  *iTE6615_InputTiming;

#define Max_SCDT_On_Count 4
u8 Current_Max_SCDT_On_Count = Max_SCDT_On_Count;

#define Max_WaitForVidIn_Count 10
u8 Current_WaitForVidIn_Count = Max_WaitForVidIn_Count;

#define Max_VideoUnstable_Count 30
u8 Current_VideoUnstable_Count = Max_VideoUnstable_Count;

#define MAX_HDCP_FAIL_COUNT 5
static u8 Current_HDCP_Fail_Count = 0;

#define MAX_SCDC_ERROR_COUNT 10
static u8 Current_SCDC_Fail_Count = 0;
...
void iTE6615_INIT_Chip(void)
{
	...
	iTE6615_HDCP_Repeater_INIT();
	iTE6615_HDCP_EnableRepeaterMode(_HDCP_Enable_iTE6615_AS_HDCPRepeater_);
    #endif

    // HDMI/DVI and decided by EDID
    iTE6615_DATA.config.EnHDMI = 1;

    iTE6615_DATA.config.AVIInfoframe_Setting_By_Array = _AVIInfoframe_Setting_By_Array_;
    ...
    iTE6615_DATA.config.Output_RGB_QuantizationRange = _Output_RGB_QuantizationRange_;    // AVI Infoframe output Quantization indicate
    iTE6615_DATA.config.Output_YUV_QuantizationRange = _Output_YUV_QuantizationRange_;    // AVI Infoframe output Quantization indicate
    ...
    iTE6615_DATA.config.Infoframe_ITContent = _Infoframe_ITContent_;
    iTE6615_DATA.config.Infoframe_ITContent_Type = _Infoframe_ITContent_Type_;
	...
	iTE6615_DATA.config.TTL_Default_YUV422ReceiveBits = _TTL_Default_YUV422ReceiveBits_;
	...
	{
		iTE6615_DATA.config.LVDS_RxChNum = LVDS4CH;
	}
    else
	{
		iTE6615_DATA.config.LVDS_RxChNum = LVDS2CH;
	}
    ...
    iTE6615_DATA.config.Infoframe_Enable_SPD = _Infoframe_Enable_SPD_;
    ...
    iTE6615_DATA.config.HDCP_Enable = 0;//_HDCP_Enable_;
    #else
	...
	iTE6615_DATA.config.Enable_VFrontProchChange = _Enable_VFrontProchChange_;

    // Other for CTS
    iTE6615_DATA.config.u8DDCSpeed = DDC75K;
...
void iTE6615_TX_IRQ(void)
{
    ...
    if (TxReg16 & 0x02) {
        ...
        if(g_bEnableIt6615PowerOffFlag == TRUE)
        {
            #if _ENABLE_SKIP_NO_AUDIO_INTERRUPT
            g_bEnableIt6615PowerOffFlag = FALSE;
            #endif
            iTE6615_aud_chg(iTE6615_STATEA_Reset);
            iTE6615_aud_chg(iTE6615_STATEA_WaitForAudIn);
        }
...
void iTE6615_HDCP_IRQ(void)
{
    ...
    // Read RxID List
    if (TxReg17 & 0x40) {
        s32 RxMsgSize, RxListTotalBytes, MsgfifoWrptr, MsgfifoRdptr, MsgfifoRdStartAdr;
        u8 *RxListID = iTE6615_DATA.KSV_List;
        u16 i;
		...
		if(RxListTotalBytes > _HDCP_KSVFifo_Size_)
		{
			HDCP_DEBUG_PRINTF(("Max KSV Fifo Size Exceeded \r\n"));
		}
		else
		{
			...
			{
				hdmitxbrd(0x5B, 5, &iTE6615_DATA.Info_HDCPDownstreamBKSV[0]);
			}

			iTE6615_DATA.Info_HDCPRxInfo[0] = hdmitxrd(0x64);
			iTE6615_DATA.Info_HDCPRxInfo[1] = hdmitxrd(0x65);
            #if _HDCP_Enable_iTE6615_AS_HDCPRepeater_Code_
			if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
			{
				iTE6615_DATA.CB_HDCP.iTE6615_Inform_HDCP2_KSVListReady_CB(&iTE6615_DATA.Info_HDCPDownstreamBKSV[0], &iTE6615_DATA.Info_HDCPRxInfo[0], &iTE6615_DATA.KSV_List[0]);
			}
            #endif
		}
...
void iTE6615_sys_fsm(void)
{
    switch(iTE6615_DATA.STATES)
    {
        ...
        case iTE6615_STATES_RxSenON:
            if(iTE6615_DATA.Flag_HDCPFailResetTX)
            {
                HDMITX_DEBUG_PRINTF(("HDCP Fail %d Times need to reset TX for compatibility !\r\n", MAX_HDCP_FAIL_COUNT));
                iTE6615_DATA.Flag_HDCPFailResetTX = 0;
                hdmitxset(0x88, 0x01, 0x01);    // disable TX output
...
void iTE6615_sys_chg(iTE6615_STATES_Type NewState)
{
    ...
    switch(iTE6615_DATA.STATES)
    {
        case iTE6615_STATES_Unplug:
            ...
            iTE6615_SetTX_Timestamp(ITE6615_DISABLE);
        ...
        case iTE6615_STATES_HPDOn:
            ...
            */
            ...
            iTE6615_SetTX_EmbeddedFormat(1);
            #endif
    ...
    switch(iTE6615_DATA.STATEV)
    {
        ...
        case iTE6615_STATEV_WaitForVidIn:
            ...
            if(Current_WaitForVidIn_Count==0)
            {
                ...
                It6615_delay1ms(100);    // delay for video stable
                #endif
            ...
            if(Current_VideoUnstable_Count==0)
            {
                Current_VideoUnstable_Count = Max_VideoUnstable_Count;
                iTE6615_DATA.Flag_Reset6615 = 1;    // for reload config for 6615+6805
                iTE6615_DATA.Flag_Reset6805 = 1;    // for reload config for 6615+6805
            }
            else
            {
                Current_VideoUnstable_Count--;
            }

        case iTE6615_STATEV_VidInStable:
            if (Current_Max_SCDT_On_Count == 0)
            {
            ...
            {
                Current_Max_SCDT_On_Count--;
                if (!iTE6615_CheckRX_SCDT())
                {
                    iTE6615_vid_chg(iTE6615_STATEV_WaitForVidIn); // no break, continue to iTE6615_STATEV_VidOutEnable
                    Current_Max_SCDT_On_Count = Max_SCDT_On_Count;
                }
            }
            break;
        case iTE6615_STATEV_VidOutEnable:
            if(Current_HDCP_Fail_Count >= MAX_HDCP_FAIL_COUNT || Current_SCDC_Fail_Count >= MAX_SCDC_ERROR_COUNT)
            {
                iTE6615_DATA.Flag_HDCPFailResetTX = 1;
                Current_HDCP_Fail_Count = 0;
                Current_SCDC_Fail_Count = 0;
            }
            break;
        default:
            break;
...
void iTE6615_vid_chg(iTE6615_STATEV_Type NewState)
{
    ...
    HDMITX_DEBUG_PRINTF(("iTE6615_vid_chg\r\n"));
    ...
    switch(iTE6615_DATA.STATEV)
    {
        case iTE6615_STATEV_Reset:
            ...
            iTE6615_SCDC_Write(0x20, 0);    // reset scramble and clock ratio
        ...
        case iTE6615_STATEV_VidInStable:
			...
			// add more SCDT check time for vidstable
            Current_Max_SCDT_On_Count = Max_SCDT_On_Count;
            Current_VideoUnstable_Count = Max_VideoUnstable_Count;
			break;
        case iTE6615_STATEV_VidOutEnable:
            ...
            #if _ENABLE_6615_SYNC_EMBEDDED_FUNCTIONS_
            iTE6615_Check_SyncEmbedded();
            #endif

            It6615_delay1ms(3);						// 20190419 add for compatibility
			...
			if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
			{
				if(iTE6615_DATA.HDCP_Enable_HDCPVersion == 0)
				{
					return;
				}
			}
...
void iTE6615_aud_chg(iTE6615_STATEA_Type NewState)
{
    ...
    HDMITX_DEBUG_PRINTF(("iTE6615_aud_chg\r\n"));
...
void iTE6615_hdcp_fsm(void)
{
    ...
    {
        case STATEH_Reset:
            //IT6615_MSG(("STATEH to STATEH_Reset\r\n"));
            break;
        case STATEH_CPGoing:
            break;
        case STATEH_Done:
            //IT6615_MSG(("STATEH to STATEH_Done\r\n"));
            break;
        case STATEH_Fail:
            //IT6615_MSG(("STATEH to STATEH_Fail\r\n"));
            break;
        default:
            //IT6615_MSG(("ERROR: STATEH to Unknown state !!!\r\n"));
            break;
    }
}
...
void iTE6615_hdcp_chg(STATEH_Type NewState)
{
    ...
    switch(iTE6615_DATA.STATEH)
    {
        ...
        case STATEH_CPGoing:
            ...
            hdmitxset(0x6F, 0x08, 0x08);    // Enable auto mute
            iTE6615_SetTX_AVMute(AVMUTE_OFF);
            iTE6615_SCDC_Set_RR(ITE6615_DISABLE);
            #if ENABLE_HDCP22_SWITCH
            if(iTE6615_GetHdcpType() == 1)
        ...
        case STATEH_Done:
			...
			if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
			{
				iTE6615_DATA.Info_HDCPStatus = IT6615_HDCP_Success;
				iTE6615_HDCP_Read(0x00, 5);
				hdmitxbrd(0x5B, 5, &iTE6615_DATA.Info_HDCPDownstreamBKSV[0]);
				iTE6615_DATA.CB_HDCP.iTE6615_Inform_HDCPDone_CB(&iTE6615_DATA.Info_HDCPDownstreamBKSV[0]);
			}
			#endif
            break;
        case STATEH_Fail:
        ...
        #if ENABLE_OUTPUT_HDCP_SWITCH
        case STATEH_Disable:
            HDMITX_DEBUG_PRINTF(("HDCP=> STATEH_Disable\r\n"));
            hdmitxset(0x6F, 0x08, 0x08);    // Enable auto mute
            iTE6615_SetTX_AVMute(AVMUTE_OFF);
            iTE6615_SCDC_Set_RR(ITE6615_DISABLE);
            iTE6615_HDCP_Disable();
            iTE6615_SCDC_Set_RR(ITE6615_ENABLE);
            //iTE6615_DATA.STATEH = STATEH_Done;
            break;
        #endif
        default:
...
// INT
#ifdef ENABLE_HPD_POLLING
u8 HPD_NOW = 3;
#endif
void iTE6615_INT_HPD_Chg(void)
{
#ifdef ENABLE_HPD_POLLING
    if (iTE6615_CheckTX_HPD() == HPD_NOW)
    {
        return;
    }
    else
    {
        HPD_NOW = iTE6615_CheckTX_HPD();
    }
#endif
...
void iTE6615_INT_SCDT_Chg(void)
{
    ...
    {
        // under my testing, sync embedded need wait more time then normal case for Video compatibility
        It6615_delay1ms(150);
    }
    else
    {
...
void iTE6615_INT_SCDC_UpdateFlagChg_Handler( u8 rddata )
{
    ...
    if (rddata & 0x02) {
        ...
        if (iTE6615_SCDC_Read(0x50, 7)) {
            ...
            CED_Total[0] = ((CED[1]) << 8) + CED[0];
            CED_Total[1] = ((CED[3]) << 8) + CED[2];
            CED_Total[2] = ((CED[5]) << 8) + CED[4];
            HDMITX_SCDC_PRINTF(("SCDC Ch0 Valid=%d, Channel 0 Error Count = 0x%04X\r\n", (CED[1] & 0x80) >> 7, CED_Total[0]));
            HDMITX_SCDC_PRINTF(("SCDC Ch1 Valid=%d, Channel 1 Error Count = 0x%04X\r\n", (CED[3] & 0x80) >> 7, CED_Total[1]));
            HDMITX_SCDC_PRINTF(("SCDC Ch2 Valid=%d, Channel 2 Error Count = 0x%04X\r\n", (CED[5] & 0x80) >> 7, CED_Total[2]));
            if(CED_Total[0] == 0xFFFF || CED_Total[1] == 0xFFFF || CED_Total[2] == 0xFFFF )
            {
                HDMITX_SCDC_PRINTF(("RX CED ERROR OCCUR = 1 ---------------------\r\n"));
                Current_SCDC_Fail_Count++;
            }
			else
			{
                HDMITX_SCDC_PRINTF(("RX CED ERROR OCCUR = 0 ---------------------\r\n"));
                Current_SCDC_Fail_Count = 0;
            }
...
u8 iTE6615_EDID_Parser( void )
{
	...
	if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
	{
		iTE6615_DATA.CB_HDCP.iTE6615_Inform_EDIDReadyBlock0_CB(edid_tmp);
	}
	#endif
    ...
    for (i = 1; i <= extblock; i++) {
		...
		if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
		{
			iTE6615_DATA.CB_HDCP.iTE6615_Inform_EDIDReadyBlock1_CB(edid_tmp);
		}
		#endif
	...
	iTE6615_DATA.data_buffer_is_EDID = 1;
...
void iTE6615_Set_Video_Infoframe(void)
{
    ...
    {
        if(iTE6615_DATA.config.AVIInfoframe_Setting_By_Array)
        {
            iTE6615_Set_AVI_Infoframe_ByArray();
        }
        else
        {
        ...
        iTE6615_Set_SPD_Infoframe();
        hdmitxset(0xC8, 0x30, 0x30);    // Enable General Control Packet    // kuro can be setting to init
    }
}

void iTE6615_Set_AVI_Infoframe_ByArray(void)
{
    u8 chksum, i;
    chgtxbank(1);

    hdmitxwr(0x58, iTE6615_DATA.Infoframe.AVI[0]);
    hdmitxwr(0x59, iTE6615_DATA.Infoframe.AVI[1]);
    hdmitxwr(0x5A, iTE6615_DATA.Infoframe.AVI[2]);
    hdmitxwr(0x5B, iTE6615_DATA.Infoframe.AVI[3]);
    hdmitxwr(0x5C, iTE6615_DATA.Infoframe.AVI[4]);
...
void iTE6615_Set_AVI_Infoframe(void)
{
    ...
    {
        // setting Coloimetry 00 = no data, 40 = 601, 80 = 709
        hdmitxset(0x59, 0xC0, iTE6615_DATA.config.Output_YCC_Colorimetry); // Colorimetry = ITE6615_ITU709
    }

    hdmitxset(0x59, 0x3F, iTE6615_DATA.config.Input_Video_AspectRatio);

	if(iTE6615_DATA.config.Infoframe_ITContent)
	{
		hdmitxset(0x5A, 0x80, 0x80);
		hdmitxset(0x5C, 0x30, iTE6615_DATA.config.Infoframe_ITContent_Type << 4);
        HDMITX_DEBUG_PRINTF(("Enable Infoframe_ITContent \r\n!!!"));
        HDMITX_DEBUG_PRINTF(("6615 setting 0x5C = 0x%02X", (int) hdmitxrd(0x5C)));
	}
	else
	{
        HDMITX_DEBUG_PRINTF(("Disable Infoframe_ITContent \r\n!!!"));
		hdmitxset(0x5A, 0x80, 0x00);
		hdmitxset(0x5C, 0x30, 0x00);
	}

    // setting Extended Colorimetry
    // need to judge EDID support ExtendedColorimetry or not
    if( iTE6615_DATA.config.Output_ExtendedColorimetry_Enable  &&
        ( ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_xvYCC601    &&  (iTE6615_DATA.sink.ExtColCap&0x01)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_xvYCC709    &&  (iTE6615_DATA.sink.ExtColCap&0x02)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_sYCC601     &&  (iTE6615_DATA.sink.ExtColCap&0x04)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_AdobeYCC601 &&  (iTE6615_DATA.sink.ExtColCap&0x08)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_AdobeRGB    &&  (iTE6615_DATA.sink.ExtColCap&0x10)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_BT2020cYCC  &&  (iTE6615_DATA.sink.ExtColCap&0x20)==0x00 ) ||
          ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_BT2020YCC   &&
          (((iTE6615_DATA.sink.ExtColCap&0x40)==0x00 && iTE6615_DATA.config.Output_ColorSpace!=ITE6615_RGB444) || ((iTE6615_DATA.sink.ExtColCap&0x80)==0x00 && iTE6615_DATA.config.Output_ColorSpace==ITE6615_RGB444))) ))
    {
          iTE6615_DATA.config.Output_ExtendedColorimetry_Enable = 0;
          HDMITX_DEBUG_PRINTF(("\nExtended Colorimetry disabled because EDID do not support !!!\r\n"));
    }
    if(iTE6615_DATA.config.Output_ExtendedColorimetry_Enable)
    {
        hdmitxset(0x59, 0xC0, 0xC0);  // Colorimetry = 11: Extended colorimetry information valid
        hdmitxset(0x5A, 0x70, iTE6615_DATA.config.Output_ExtendedColorimetry_Format); // ColorimetryExtended
    }

    // Setting VIC
    #if _ENABLE_4K_FORCE_SET_VIC_0       // wf8421 20190401
    ...
    hdmitxwr(0x5C, (iTE6615_DATA.config.Output_YUV_QuantizationRange << 6 ) + iTE6615_DATA.config.PixRpt - 1);
...
void iTE6615_Set_DRM_Infoframe()
{
    ...
    for (i = 0x3C; i <= 0x55; i++)
    {
        if (iTE6615_DATA.config.Infoframe_DRM_ZERO)
            ...
            hdmitxwr(i, iTE6615_DATA.Infoframe.DRM[i-0x3C]); // dummy content for test
    }
    ...
    if (iTE6615_DATA.config.Output_ExtendedColorimetry_Format == ITE6615_xvYCC601
    || iTE6615_DATA.config.Output_ExtendedColorimetry_Format == ITE6615_xvYCC709)
    {
        chgtxbank(1);
        hdmitxwr(0xB0, 0x00);
        hdmitxwr(0xB1, 0xB0);
...
void iTE6615_Set_SPD_Infoframe()
{
    u8 i;

    if(!iTE6615_DATA.config.Infoframe_Enable_SPD)
    {
        chgtxbank(0);
        hdmitxset(0xC8, 0x0C, 0x00);    // Disable SPD PKT (using ACP PKT register implement it)
        return;
    }
    ...
    for (i = 0x70; i <= 0x8E; i++)
    {
        hdmitxwr(i, iTE6615_DATA.Infoframe.SPD[i-0x70]);
    }

    chgtxbank(0);
    hdmitxset(0xC8, 0x0C, 0x0C);     // Enable AVI InfoFrame
}
...
void iTE6615_Set_VendorSpecific_Infoframe()
{
    ...
    if( iTE6615_DATA.config.Output_ExtendedColorimetry_Enable == 1 &&
      ( iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_xvYCC601 ||
        iTE6615_DATA.config.Output_ExtendedColorimetry_Format==ITE6615_xvYCC709 ) ) {
        chgtxbank(1);
        hdmitxwr(0xB0, 0x00);
        hdmitxwr(0xB1, 0xB0);
...
void iTE6615_Set_Video_ColorSpace(void)
{
    ...
    if (iTE6615_DATA.config.Output_ColorDepth == VID8BIT || iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV422)
        hdmitxset(0xC1, 0x70, 0);
    else
        hdmitxset(0xC1, 0x70, (0x04 + iTE6615_DATA.config.Output_ColorDepth) << 4);

    // CSC
    if (iTE6615_DATA.config.Input_ColorSpace  == ITE6615_RGB444 &&
       (iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV444 ||
        iTE6615_DATA.config.Output_ColorSpace == ITE6615_YUV422))
    {
        // Input RGB to YUV
        ...
        {
            case RGB_RANGE_FULL:
                if(iTE6615_DATA.config.Output_YCC_Colorimetry == ITE6615_ITU601)
                    CSCMtx_Type = CSCMtx_RGB2YUV_ITU601_00_255; // RGB Full Range to ITU601
                else
                    CSCMtx_Type = CSCMtx_RGB2YUV_ITU709_00_255; // RGB Full Range to ITE6615_ITU709
                break;
    ...
    else if ((iTE6615_DATA.config.Input_ColorSpace == ITE6615_YUV444 ||
              iTE6615_DATA.config.Input_ColorSpace == ITE6615_YUV422 ||
              iTE6615_DATA.config.Input_ColorSpace == ITE6615_YUV420) &&
              iTE6615_DATA.config.Output_ColorSpace == ITE6615_RGB444)
    {
        ...
        {
            case RGB_RANGE_FULL:
                if(iTE6615_DATA.config.Input_YCC_Colorimetry == ITE6615_ITU601)
                {
                    CSCMtx_Type = CSCMtx_YUV2RGB_ITU601_00_255;
                }
                else if(iTE6615_DATA.config.Input_YCC_Colorimetry == ITE6615_ITU709)
                {
...
void iTE6615_Set_HDMI2_SCDC( void )
{
	...
	HDMITX_DEBUG_PRINTF(("iTE6615_DATA.sink.lte_340M_csc_scramble = %d \r\n", iTE6615_DATA.sink.lte_340M_csc_scramble));
    ...
    if ((iTE6615_DATA.sink.HDMIVer >= 2) &&  iTE6615_DATA.sink.scdc_present)
    {
        iTE6615_SCDC_Set_RR(ITE6615_DISABLE);
    }
    UNUSED(rddata);
...
{
    if(iTE6615_InputTiming == NULL) return;
    if(iTE6615_DATA.config.TTL_Enable_SyncEmbedd==0) return;

    if(iTE6615_InputTiming->VTotal != iTE6615_CurVTiming.VTotal ||
       iTE6615_InputTiming->VFrontPorch != iTE6615_CurVTiming.VFrontPorch)
    {

        if((iTE6615_InputTiming+1)->VTotal == iTE6615_CurVTiming.VTotal &&
           //(iTE6615_InputTiming+1)->VFrontPorch == iTE6615_CurVTiming.VFrontPorch &&
            iTE6615_InputTiming->VIC == (iTE6615_InputTiming+1)->VIC)
        {
            iTE6615_InputTiming++;
            iTE6615_SetTX_EmbeddedFormat(0);
            return;
        }

        if((iTE6615_InputTiming+2)->VTotal == iTE6615_CurVTiming.VTotal &&
           //(iTE6615_InputTiming+1)->VFrontPorch == iTE6615_CurVTiming.VFrontPorch &&
            iTE6615_InputTiming->VIC == (iTE6615_InputTiming+2)->VIC)
        {
            iTE6615_InputTiming = iTE6615_InputTiming+2;
            iTE6615_SetTX_EmbeddedFormat(0);
        }
                ...
                case CMD_HDCP_KSV_CHK:
                    ...
                    if(iTE6615_HDCPRepeater_CheckSupport() == TRUE)
                    {
						if((iTE6615_DATA.Info_HDCPBStatus[0]&0x80) || (iTE6615_DATA.Info_HDCPBStatus[1]&0x08))
						{
							...
							HDCP_DEBUG_PRINTF(("iTE: SHA Check Result = FAIL Because MAX_DEVS_EXCEEDED or MAX_CASCADE_EXCEEDED \r\n"));
						}
						else
						{
						...
						if(iTE6615_DATA.HDCP_Enable_iTE6615_AS_HDCPRepeater == 1)
						{
							iTE6615_DATA.CB_HDCP.iTE6615_Inform_HDCP1_KSVListReady_CB(&iTE6615_DATA.Info_HDCPDownstreamBKSV, &iTE6615_DATA.Info_HDCPBStatus, &iTE6615_DATA.KSV_List[0]);
						}
                        #endif
                    }
                    else{
...
void iTE6615_TASK_HDCP_RetryAuth( void )
{
    ...
    iTE6615_SetTX_Timestamp(ITE6615_ENABLE);


    Current_HDCP_Fail_Count++;
    HDMITX_DEBUG_PRINTF(("HDCP Fail Count = %d \r\n", (int) Current_HDCP_Fail_Count ));
    iTE6615_Get_TXCLK();    // for get the setting of AFE
...
void iTE6615_GetTxEdid(u8 *pu8EdidBuf)
{
    iTE6615_Get_EDID_Block(0,pu8EdidBuf);
    if(pu8EdidBuf[0x7E]!=0)
    {
        iTE6615_Get_EDID_Block(1,pu8EdidBuf+128);
    }
}
